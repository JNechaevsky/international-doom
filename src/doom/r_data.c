...
void R_InitColormaps (void)
{
	...
	// [crispy] Invulnerability (c == COLORMAPS)
    {
        ...
        for (int i = 0; i < 256; i++)
        {
            ...
            pal[0] = (byte)BETWEEN(0, 255, (int)(src_r * vid_r_intensity));
            pal[1] = (byte)BETWEEN(0, 255, (int)(src_g * vid_g_intensity));
            pal[2] = (byte)BETWEEN(0, 255, (int)(src_b * vid_b_intensity));
...
static void R_InitHSVColors (void)
{
    ...
    int lump = W_CheckNumForName(DEH_String("CRGREEN"));
    if (lump >= 0)
        cr[CR_RED2GREEN] = W_CacheLumpNum(lump, PU_STATIC);

    lump = W_CheckNumForName(DEH_String("CRBLUE2"));
    if (lump == -1)
        lump = W_CheckNumForName(DEH_String("CRBLUE"));
    if (lump >= 0)
