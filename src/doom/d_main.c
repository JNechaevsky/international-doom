...
static void D_Display (void)
{
    ...
    do
    {
        do
        {
            ...
            I_Sleep(1);
#ifndef CRISPY_TRUECOLOR
        // [JN] Note: in paletted render (vid_screenwipe == 3) tics are counting
        // slower, since the effect can't be smooth because of palette limitation.
...
boolean D_GrabMouseCallback(void)
{
    ...
    if ((gamestate == GS_INTERMISSION || gamestate == GS_FINALE || gamestate == GS_THEEND)
...
//    W_PrintDirectory();
    ...
    if (!M_ParmExists("-noautoload") && gamemode != shareware
    && autoload_deh == 2)  // [JN] Allow autoload per PWAD only.
    {
        int i;
