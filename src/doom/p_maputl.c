  ...
  boolean(*func)(mobj_t*) )
{
    ...
    if (singleplayer && compat_blockmap_fix)
    {
...
PIT_AddLineIntercepts (line_t* ld)
{
    ...
    if (intercept_p - intercepts == MAXINTERCEPTS_ORIGINAL + 1)
    {
        ...
        else
        {
            ...
            printf("PIT_AddLineIntercepts: Triggered INTERCEPTS overflow.\n");
        }
    }
    else
...
//
static boolean
PIT_CompatAddLineIntercepts(line_t *const ld)
{
    int s1;
    int s2;
    fixed_t frac;
...
static boolean PIT_AddThingIntercepts (mobj_t* thing)
{
    ...
    if (intercept_p - intercepts == MAXINTERCEPTS_ORIGINAL + 1)
    {
        ...
        {
            // [crispy] print a warning
            printf("PIT_AddThingIntercepts: Triggered INTERCEPTS overflow.\n");
        }
    }
    else
    if (intercept_p - intercepts >= MAXINTERCEPTS_ALLGHOSTS) // 147
