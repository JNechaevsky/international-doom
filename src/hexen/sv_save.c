...
static byte SavePixelToPalette(pixel_t pixel);
static boolean BuildSavePreview(byte *thumb);
...
static byte save_preview_cache[SAVEGAME_PREVIEW_SIZE];
static boolean save_preview_cache_valid = false;
static boolean save_preview_capture_requested = false;
...
static boolean BuildSavePreview(byte *thumb)
{
    ...
    for (int y = 0; y < SAVEGAME_PREVIEW_HEIGHT; ++y)
    {
        ...
        for (int x = 0; x < SAVEGAME_PREVIEW_WIDTH; ++x)
        {
            ...
            thumb[y * SAVEGAME_PREVIEW_WIDTH + x] = SavePixelToPalette(px);
        }
    }
...
{
    save_preview_cache_valid = false;
    save_preview_capture_requested = true;
}

boolean SV_IsSavePreviewReady(void)
{
    return !save_preview_capture_requested;
}

// [PN] Refresh clean world-only save preview cache from the freshly rendered view.
void SV_UpdateSavePreviewCache(void)
{
    if (!save_preview_capture_requested)
    {
        return;
    }

    save_preview_cache_valid = BuildSavePreview(save_preview_cache);
    save_preview_capture_requested = false;
}

// [PN] Append fixed-size preview and "ISVP" footer to save tail.
static void ArchiveSavePreview(void)
{
    byte thumb[SAVEGAME_PREVIEW_SIZE] = {0};

    // [PN] Always write a preview block: use cached world image or black fallback.
    if (save_preview_cache_valid)
    {
        memcpy(thumb, save_preview_cache, sizeof(thumb));
    }
    else
    {
    ...
    // [PN] Keep format simple: raw paletted thumbnail + compact footer.
    // [PN] "ISVP" = "Inter Save View Preview".
    SV_Write(thumb, SAVEGAME_PREVIEW_SIZE);
    SV_WriteByte('I');
    SV_WriteByte('S');
    SV_WriteByte('V');
    SV_WriteByte('P');
...
static void UnarchiveMobjs(void)
{
    ...
    for (i = 0; i < MobjCount; i++)
    {
        ...
        // [JN] Demo critical! Do not apply Lee Killough's fix
        // for falling down into a wall after savegame loaded.
...
static void CopyFile(char *source_name, char *dest_name)
{
    ...
    file_remaining = M_FileLength(read_handle);
