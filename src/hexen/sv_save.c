...
{
    V_SavePreview_RequestCapture(&save_preview_cache);
}

boolean SV_IsSavePreviewReady(void)
{
    return V_SavePreview_IsReady(&save_preview_cache);
}

// [PN] Refresh clean world-only save preview cache from the freshly rendered view.
void SV_UpdateSavePreviewCache(void)
{
    if (!save_preview_cache.capture_requested)
    {
        return;
    }

    V_SavePreview_UpdateCache(&save_preview_cache,
                               I_VideoBuffer,
                               SCREENWIDTH,
                               SCREENHEIGHT,
                               viewwindowx,
...
static void ArchiveSavePreview(void)
{
    ...
    V_SavePreview_CopyOrBlack(&save_preview_cache, thumb);
    V_SavePreview_WriteFooter(footer);

    SV_Write(thumb, V_SAVEPREVIEW_SIZE);
    SV_Write(footer, V_SAVEPREVIEW_FOOTER_SIZE);
}
