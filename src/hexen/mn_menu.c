...
typedef enum
{
    ...
    MENU_ID_MOUSEBINDS2,
    MENU_ID_MOUSEBINDS3,
    MENU_ID_GAMEPADBINDS,
    MENU_ID_GAMEPADBINDS2,
    MENU_ID_GAMEPADSETTINGS1,
    MENU_ID_GAMEPADSETTINGS2,
    MENU_ID_WIDGETS,
    MENU_ID_AUTOMAP,
    MENU_ID_GAMEPLAY1,
    MENU_ID_GAMEPLAY2,
...
static void M_Bind_Rewind (int choice);
...
static void M_Draw_ID_MouseBinds1 (void);
...
static void M_Draw_ID_MouseBinds2 (void);
static void M_Bind_M_Quartz (int choice);
static void M_Bind_M_Urn (int choice);
static void M_Bind_M_Flechette (int choice);
static void M_Bind_M_Disk (int choice);
static void M_Bind_M_Icon (int choice);
static void M_Bind_M_Porkalator (int choice);
static void M_Bind_M_Chaos (int choice);
static void M_Bind_M_Banishment (int choice);
static void M_Bind_M_Wings (int choice);
static void M_Bind_M_Servant (int choice);
static void M_Bind_M_Bracers (int choice);
static void M_Bind_M_Boots (int choice);
static void M_Bind_M_Torch (int choice);
static void M_Bind_M_Krater (int choice);
static void M_Bind_M_Incant (int choice);
static void M_Bind_M_AllArti (int choice);

static void M_Draw_ID_MouseBinds3 (void);

static void M_Bind_M_Reset (int choice);

static void M_Draw_ID_GamepadBinds (void);
static void M_Draw_ID_GamepadBinds_2 (void);
...
static void M_Bind_G_Jump (int choice);
...
static void M_Bind_G_InventoryLeft (int choice);
static void M_Bind_G_InventoryRight (int choice);
static void M_Bind_G_UseArtifact (int choice);
static void M_Bind_G_FlyUp (int choice);
static void M_Bind_G_FlyDown (int choice);
static void M_Bind_G_FlyCenter (int choice);
...
static Menu_t ID_Def_MouseBinds1;
static Menu_t ID_Def_MouseBinds2;
static Menu_t ID_Def_MouseBinds3;
static Menu_t ID_Def_GamepadBinds;
static Menu_t ID_Def_GamepadBinds_2;
...
static void M_ScrollPages (boolean direction)
{
    ...
    // Mouse bindings
    else if (CurrentMenu == &ID_Def_MouseBinds1) nextMenu = (direction ? MENU_ID_MOUSEBINDS2 : MENU_ID_MOUSEBINDS3);
    else if (CurrentMenu == &ID_Def_MouseBinds2) nextMenu = (direction ? MENU_ID_MOUSEBINDS3 : MENU_ID_MOUSEBINDS1);
    else if (CurrentMenu == &ID_Def_MouseBinds3) nextMenu = (direction ? MENU_ID_MOUSEBINDS1 : MENU_ID_MOUSEBINDS2);

    // Gamepad bindings:
    else if (CurrentMenu == &ID_Def_GamepadBinds) nextMenu = (direction ? MENU_ID_GAMEPADBINDS2 : MENU_ID_GAMEPADSETTINGS2);
    else if (CurrentMenu == &ID_Def_GamepadBinds_2) nextMenu = (direction ? MENU_ID_GAMEPADSETTINGS1 : MENU_ID_GAMEPADBINDS);
    else if (CurrentMenu == &ID_Def_GamepadSettings_1) nextMenu = (direction ? MENU_ID_GAMEPADSETTINGS2 : MENU_ID_GAMEPADBINDS2);
    else if (CurrentMenu == &ID_Def_GamepadSettings_2) nextMenu = (direction ? MENU_ID_GAMEPADBINDS : MENU_ID_GAMEPADSETTINGS1);
...
static void M_Draw_ID_Video_1 (void)
{
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 20,
                        ...
                        vid_truecolor == 2 ? cr[CR_GREEN_HX] : cr[CR_DARKRED], 
                            vid_truecolor == 1 ? cr[CR_YELLOW_BRIGHT] :
                            vid_truecolor == 2 ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT], 
                                LINE_ALPHA(0));
...
static MenuItem_t ID_Menu_Controls[] = {
    ...
    { ITT_EFUNC,   "MOUSE BINDINGS",           M_Choose_ID_Mousebinds,         0, MENU_NONE },
    { ITT_EFUNC,   "GAMEPAD SETTINGS",         M_Choose_ID_GamepadBinds,       0, MENU_NONE },
    { ITT_EMPTY,   NULL,                       NULL,                           0, MENU_NONE },
...
static void M_Draw_ID_Controls (void)
{
    ...
    MN_DrTextACentered("CONTROLS CONFIGURATION", 10, cr[CR_YELLOW]);

    MN_DrTextACentered("MOUSE CONFIGURATION", 50, cr[CR_YELLOW]);

    DrawSlider(&ID_Def_Controls, 5, 16, mouseSensitivity, false, 4);
    M_ID_HandleSliderMouseControl(66, 70, 132, &mouseSensitivity, false, 0, 15);
    sprintf(str,"%d", mouseSensitivity);
    MN_DrTextAGlow(str, 227, 75,
    ...
    DrawSlider(&ID_Def_Controls, 8, 16, mouse_sensitivity_y, false, 7);
    M_ID_HandleSliderMouseControl(66, 100, 132, &mouse_sensitivity_y, false, 0, 15);
    sprintf(str,"%d", mouse_sensitivity_y);
    MN_DrTextAGlow(str, 227, 105,
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 180,
...
static void M_Draw_ID_Keybinds_1 (void)
{
    ...
    M_DrawBindKey(8, 100, key_jump, key_jump2);
    M_DrawBindKey(9, 110, key_180turn, key_180turn2);

    MN_DrTextACentered("ACTION", 120, cr[CR_YELLOW]);

    M_DrawBindKey(11, 130, key_fire, key_fire2);
    M_DrawBindKey(12, 140, key_use, key_use2);

    M_DrawBindFooter("PAGE 1/8", true);
}
...
static void M_Draw_ID_Keybinds_2 (void)
{
    ...
    M_DrawBindFooter("PAGE 2/8", true);
}
...
static void M_Draw_ID_Keybinds_3 (void)
{
    ...
    M_DrawBindFooter("PAGE 3/8", true);
}
...
static void M_Draw_ID_Keybinds_4 (void)
{
    ...
    M_DrawBindKey(1, 30, key_speed_up, key_speed_up2);
    M_DrawBindKey(2, 40, key_speed_down, key_speed_down2);
    M_DrawBindKey(3, 50, key_speed_reset, key_speed_reset2);
    M_DrawBindKey(4, 60, key_rewind, key_rewind2);
    ...
    M_DrawBindKey(10, 120, key_prevweapon, key_prevweapon2);
    M_DrawBindKey(11, 130, key_nextweapon, key_nextweapon2);

    MN_DrTextACentered("ARTIFACTS", 140, cr[CR_YELLOW]);

    M_DrawBindKey(13, 150, key_arti_health, key_arti_health2);
    M_DrawBindKey(14, 160, key_arti_urn, key_arti_urn2);

    M_DrawBindFooter("PAGE 4/8", true);
}

static void M_Bind_ShortTics (int choice)
{
    M_StartBind(400);  // key_shorttics
}

static void M_Bind_IncreaseGameSpeed (int choice)
{
    M_StartBind(401);  // key_speed_up
}

static void M_Bind_DecreaseGameSpeed (int choice)
{
    M_StartBind(402);  // key_speed_down
}

static void M_Bind_ResetGameSpeed (int choice)
{
    M_StartBind(403);  // key_speed_reset
}

static void M_Bind_Rewind (int choice)
{
    M_StartBind(404);  // key_rewind
}

static void M_Bind_Weapon1 (int choice)
{
    M_StartBind(405);  // key_weapon1
}

static void M_Bind_Weapon2 (int choice)
{
    M_StartBind(406);  // key_weapon2
}

static void M_Bind_Weapon3 (int choice)
{
    M_StartBind(407);  // key_weapon3
}

static void M_Bind_Weapon4 (int choice)
{
    M_StartBind(408);  // key_weapon4
}

static void M_Bind_PrevWeapon (int choice)
{
    M_StartBind(409);  // key_prevweapon
}

static void M_Bind_NextWeapon (int choice)
{
    M_StartBind(410);  // key_nextweapon
}

static void M_Bind_Quartz (int choice)
{
    M_StartBind(411);  // key_arti_health
}

static void M_Bind_Urn (int choice)
{
    M_StartBind(412);  // key_arti_urn
}
...
static void M_Draw_ID_Keybinds_5 (void)
{
    ...
    M_DrawBindKey(0, 20, key_arti_poisonbag, key_arti_poisonbag2);
    M_DrawBindKey(1, 30, key_arti_blastradius, key_arti_blastradius2);
    M_DrawBindKey(2, 40, key_arti_invulnerability, key_arti_invulnerability2);
    M_DrawBindKey(3, 50, key_arti_egg, key_arti_egg2);
    M_DrawBindKey(4, 60, key_arti_teleport, key_arti_teleport2);
    M_DrawBindKey(5, 70, key_arti_teleportother, key_arti_teleportother2);
    M_DrawBindKey(6, 80, key_arti_wings, key_arti_wings2);
    M_DrawBindKey(7, 90, key_arti_servant, key_arti_servant2);
    M_DrawBindKey(8, 100, key_arti_bracers, key_arti_bracers2);
    M_DrawBindKey(9, 110, key_arti_boots, key_arti_boots2);
    M_DrawBindKey(10, 120, key_arti_torch, key_arti_torch2);
    M_DrawBindKey(11, 130, key_arti_krater, key_arti_krater2);
    M_DrawBindKey(12, 140, key_arti_incant, key_arti_incant2);
    M_DrawBindKey(13, 150, key_arti_all, key_arti_all2);

    M_DrawBindFooter("PAGE 5/8", true);
}

static void M_Bind_Flechette (int choice)
{
    M_StartBind(500);  // key_arti_poisonbag
}

static void M_Bind_Disk (int choice)
{
    M_StartBind(501);  // key_arti_blastradius
}

static void M_Bind_Icon (int choice)
{
    M_StartBind(502);  // key_arti_invulnerability
}

static void M_Bind_Porkalator (int choice)
{
    M_StartBind(503);  // key_arti_egg
}

static void M_Bind_Chaos (int choice)
{
    M_StartBind(504);  // key_arti_teleport
}

static void M_Bind_Banishment (int choice)
{
    M_StartBind(505);  // key_arti_teleportother
}

static void M_Bind_Wings (int choice)
{
    M_StartBind(506);  // key_arti_wings
}

static void M_Bind_Servant (int choice)
{
    M_StartBind(507);  // key_arti_servant
}

static void M_Bind_Bracers (int choice)
{
    M_StartBind(508);  // key_arti_bracers
}

static void M_Bind_Boots (int choice)
{
    M_StartBind(509);  // key_arti_boots
}

static void M_Bind_Torch (int choice)
{
    M_StartBind(510);  // key_arti_torch
}

static void M_Bind_Krater (int choice)
{
    M_StartBind(511);  // key_arti_krater
}

static void M_Bind_Incant (int choice)
{
    M_StartBind(512);  // key_arti_incant
}

static void M_Bind_AllArti (int choice)
{
    M_StartBind(513);  // key_arti_all
}
...
static MenuItem_t ID_Menu_Keybinds_6[] = {
    ...
    { ITT_EFUNC, "TOGGLE MINIMAP",     M_Bind_ToggleMini,  0, MENU_NONE },
...
static void M_Draw_ID_Keybinds_6 (void)
{
    ...
    M_DrawBindKey(1, 30, key_map_mini, key_map_mini2);
    M_DrawBindKey(2, 40, key_map_zoomin, key_map_zoomin2);
    M_DrawBindKey(3, 50, key_map_zoomout, key_map_zoomout2);
    M_DrawBindKey(4, 60, key_map_maxzoom, key_map_maxzoom2);
    M_DrawBindKey(5, 70, key_map_follow, key_map_follow2);
    M_DrawBindKey(6, 80, key_map_rotate, key_map_rotate2);
    M_DrawBindKey(7, 90, key_map_overlay, key_map_overlay2);
    M_DrawBindKey(8, 100, key_map_mousepan, key_map_mousepan2);
    ...
    M_DrawBindFooter("PAGE 6/8", true);
}
...
static void M_Bind_ToggleMini (int choice)
{
    M_StartBind(601);  // key_map_mini
}

static void M_Bind_ZoomIn (int choice)
{
    M_StartBind(602);  // key_map_zoomin
}

static void M_Bind_ZoomOut (int choice)
{
    M_StartBind(603);  // key_map_zoomout
}

static void M_Bind_MaxZoom (int choice)
{
    M_StartBind(604);  // key_map_maxzoom
}

static void M_Bind_FollowMode (int choice)
{
    M_StartBind(605);  // key_map_follow
}

static void M_Bind_RotateMode (int choice)
{
    M_StartBind(606);  // key_map_rotate
}

static void M_Bind_OverlayMode (int choice)
{
    M_StartBind(607);  // key_map_overlay
}

static void M_Bind_PanMode (int choice)
{
    M_StartBind(608);  // key_map_mousepan
}

static void M_Bind_ToggleGrid (int choice)
{
    M_StartBind(609);  // key_map_grid
}

static void M_Bind_AddMark (int choice)
{
    M_StartBind(610);  // key_map_mark
}

static void M_Bind_ClearMark (int choice)
{
    M_StartBind(611);  // key_map_clearmark
}
...
static void M_Draw_ID_Keybinds_7 (void)
{
    ...
    M_DrawBindKey(10, 120, key_menu_gammad, key_menu_gammad2);
    M_DrawBindKey(11, 130, key_menu_gamma, key_menu_gamma2);
    M_DrawBindKey(12, 140, key_spy, key_spy2);

    M_DrawBindFooter("PAGE 7/8", true);
}
...
static void M_Bind_DecreaseGamma (int choice)
{
    M_StartBind(710);  // key_menu_gammad
}

static void M_Bind_IncreaseGamma (int choice)
{
    M_StartBind(711);  // key_menu_gamma
}

static void M_Bind_MultiplayerSpy (int choice)
{
    M_StartBind(712);  // key_spy
}
...
static void M_Draw_ID_Keybinds_8 (void)
{
    ...
    M_DrawBindKey(3, 50, key_message_refresh_hr, key_message_refresh_hr2);
    M_DrawBindKey(4, 60, key_demo_quit, key_demo_quit2);
	M_DrawBindKey(5, 70, key_switch_ost, key_switch_ost2);

    MN_DrTextACentered("MULTIPLAYER", 80, cr[CR_YELLOW]);

    M_DrawBindKey(7, 90, key_multi_msg, key_multi_msg2);

    MN_DrTextACentered("RESET", 100, cr[CR_YELLOW]);
    ...
    */
    ...
    M_DrawBindFooter("PAGE 8/8", true);
}
...
static void M_Bind_SaveCleanshot (int choice)
{
    M_StartBind(802);  // key_menu_cleanshot
}

static void M_Bind_LastMessage (int choice)
{
    M_StartBind(803);  // key_message_refresh_hr
}

static void M_Bind_FinishDemo (int choice)
{
    M_StartBind(804);  // key_demo_quit
}

static void M_Bind_SwitchOST (int choice)
{
    M_StartBind(805);  // key_switch_ost
}

static void M_Bind_SendMessage (int choice)
{
    M_StartBind(806);  // key_multi_msg
}
...
static MenuItem_t ID_Menu_MouseBinds1[] = {
...
static Menu_t ID_Def_MouseBinds1 = {
    ID_MENU_CTRLSOFFSET, ID_MENU_TOPOFFSET,
    M_Draw_ID_MouseBinds1,
    ITEMCOUNT(ID_Menu_MouseBinds1), ID_Menu_MouseBinds1,
    0,
    SmallFont, true, true,
    MENU_ID_CONTROLS
};
...
static void M_Draw_ID_MouseBinds1 (void)
{
    Mousebinds_Cur = (MenuType_t)MENU_ID_MOUSEBINDS1;
    ...
    M_DrawBindButton(4, 60, mousebjump, mousebjump2);
    M_DrawBindButton(5, 70, mousebspeed, mousebspeed2);
    M_DrawBindButton(6, 80, mousebstrafe, mousebstrafe2);
    M_DrawBindButton(7, 90, mousebstrafeleft, mousebstrafeleft2);
    M_DrawBindButton(8, 100, mousebstraferight, mousebstraferight2);
    M_DrawBindButton(9, 110, mousebprevweapon, mousebprevweapon2);
    M_DrawBindButton(10, 120, mousebnextweapon, mousebnextweapon2);

    M_DrawBindFooter("PAGE 1/3", true);
}

static void M_Bind_M_FireAttack (int choice)     { M_StartMouseBind(1000); } // mousebfire
static void M_Bind_M_MoveForward (int choice)    { M_StartMouseBind(1001); } // mousebforward
static void M_Bind_M_MoveBackward (int choice)   { M_StartMouseBind(1002); } // mousebbackward
static void M_Bind_M_Use (int choice)            { M_StartMouseBind(1003); } // mousebuse
static void M_Bind_M_Jump (int choice)           { M_StartMouseBind(1004); } // mousebjump
static void M_Bind_M_SpeedOn (int choice)        { M_StartMouseBind(1005); } // mousebspeed
static void M_Bind_M_StrafeOn (int choice)       { M_StartMouseBind(1006); } // mousebstrafe
static void M_Bind_M_StrafeLeft (int choice)     { M_StartMouseBind(1007); } // mousebstrafeleft
static void M_Bind_M_StrafeRight (int choice)    { M_StartMouseBind(1008); } // mousebstraferight
static void M_Bind_M_PrevWeapon (int choice)     { M_StartMouseBind(1009); } // mousebprevweapon
static void M_Bind_M_NextWeapon (int choice)     { M_StartMouseBind(1010); } // mousebnextweapon
...
static MenuItem_t ID_Menu_MouseBinds2[] = {
    ...
    { ITT_EFUNC, "FLECHETTE",            M_Bind_M_Flechette,      0, MENU_NONE },
    { ITT_EFUNC, "DISK OF REPULSTION",   M_Bind_M_Disk,           0, MENU_NONE },
    { ITT_EFUNC, "ICON OF THE DEFENDER", M_Bind_M_Icon,           0, MENU_NONE },
    { ITT_EFUNC, "PORKALATOR",           M_Bind_M_Porkalator,     0, MENU_NONE },
    { ITT_EFUNC, "CHAOS DEVICE",         M_Bind_M_Chaos,          0, MENU_NONE },
    { ITT_EFUNC, "BANISHMENT DEVICE",    M_Bind_M_Banishment,     0, MENU_NONE },
    { ITT_EFUNC, "WINGS OF WRATH",       M_Bind_M_Wings,          0, MENU_NONE },
    { ITT_EFUNC, "DARK SERVANT",         M_Bind_M_Servant,        0, MENU_NONE },
    { ITT_EFUNC, "DRAGONSKIN BRACERS",   M_Bind_M_Bracers,        0, MENU_NONE },
};

static Menu_t ID_Def_MouseBinds2 = {
    ID_MENU_CTRLSOFFSET, ID_MENU_TOPOFFSET,
    M_Draw_ID_MouseBinds2,
    ITEMCOUNT(ID_Menu_MouseBinds2), ID_Menu_MouseBinds2,
    0,
    SmallFont, true, true,
    MENU_ID_CONTROLS
};
...
static void M_Draw_ID_MouseBinds2 (void)
{
    Mousebinds_Cur = (MenuType_t)MENU_ID_MOUSEBINDS2;

    M_FillBackground();

    MN_DrTextACentered("ARTIFACTS", 10, cr[CR_YELLOW]);

    M_DrawBindButton(0, 20, mousebinvleft, mousebinvleft2);
    M_DrawBindButton(1, 30, mousebinvright, mousebinvright2);
    M_DrawBindButton(2, 40, mousebuseartifact, mousebuseartifact2);
    M_DrawBindButton(3, 50, mouseb_arti_health, mouseb_arti_health2);
    M_DrawBindButton(4, 60, mouseb_arti_urn, mouseb_arti_urn2);
    M_DrawBindButton(5, 70, mouseb_arti_poisonbag, mouseb_arti_poisonbag2);
    M_DrawBindButton(6, 80, mouseb_arti_blastradius, mouseb_arti_blastradius2);
    M_DrawBindButton(7, 90, mouseb_arti_invulnerability, mouseb_arti_invulnerability2);
    M_DrawBindButton(8, 100, mouseb_arti_egg, mouseb_arti_egg2);
    M_DrawBindButton(9, 110, mouseb_arti_teleport, mouseb_arti_teleport2);
    M_DrawBindButton(10, 120, mouseb_arti_teleportother, mouseb_arti_teleportother2);
    M_DrawBindButton(11, 130, mouseb_arti_wings, mouseb_arti_wings2);
    M_DrawBindButton(12, 140, mouseb_arti_servant, mouseb_arti_servant2);
    M_DrawBindButton(13, 150, mouseb_arti_bracers, mouseb_arti_bracers2);

    M_DrawBindFooter("PAGE 2/3", true);
}

static void M_Bind_M_InventoryLeft (int choice)  { M_StartMouseBind(1100); } // mousebinvleft
static void M_Bind_M_InventoryRight (int choice) { M_StartMouseBind(1101); } // mousebinvright
static void M_Bind_M_UseArtifact (int choice)    { M_StartMouseBind(1102); } // mousebuseartifact
static void M_Bind_M_Quartz (int choice)         { M_StartMouseBind(1103); } // mouseb_arti_quartz
static void M_Bind_M_Urn (int choice)            { M_StartMouseBind(1104); } // mouseb_arti_urn
static void M_Bind_M_Flechette (int choice)      { M_StartMouseBind(1105); } // mouseb_arti_bomb
static void M_Bind_M_Disk (int choice)           { M_StartMouseBind(1106); } // mouseb_arti_blastradius
static void M_Bind_M_Icon (int choice)           { M_StartMouseBind(1107); } // mouseb_arti_invulnerability
static void M_Bind_M_Porkalator (int choice)     { M_StartMouseBind(1108); } // mouseb_arti_egg
static void M_Bind_M_Chaos (int choice)          { M_StartMouseBind(1109); } // mouseb_arti_teleport
static void M_Bind_M_Banishment (int choice)     { M_StartMouseBind(1110); } // mouseb_arti_teleportother
static void M_Bind_M_Wings (int choice)          { M_StartMouseBind(1111); } // mouseb_arti_wings
static void M_Bind_M_Servant (int choice)        { M_StartMouseBind(1112); } // mouseb_arti_servant
static void M_Bind_M_Bracers (int choice)        { M_StartMouseBind(1113); } // mouseb_arti_bracers

// -----------------------------------------------------------------------------
// Mouse bindings 3
// -----------------------------------------------------------------------------

static MenuItem_t ID_Menu_MouseBinds3[] = {
    { ITT_EFUNC, "BOOTS OF SPEED",            M_Bind_M_Boots,   0, MENU_NONE },
    { ITT_EFUNC, "TORCH",                     M_Bind_M_Torch,   0, MENU_NONE },
    { ITT_EFUNC, "KRATER OF MIGHT",           M_Bind_M_Krater,  0, MENU_NONE },
    { ITT_EFUNC, "MYSTIC AMBIT INCANT",       M_Bind_M_Incant,  0, MENU_NONE },
    { ITT_EFUNC, "ONE OF EACH",               M_Bind_M_AllArti, 0, MENU_NONE },
    { ITT_EMPTY, NULL,                        NULL,             0, MENU_NONE },
...
static Menu_t ID_Def_MouseBinds3 = {
    ID_MENU_CTRLSOFFSET, ID_MENU_TOPOFFSET,
    M_Draw_ID_MouseBinds3,
    ITEMCOUNT(ID_Menu_MouseBinds3), ID_Menu_MouseBinds3,
    0,
    SmallFont, true, true,
    MENU_ID_CONTROLS
};
...
static void M_Draw_ID_MouseBinds3 (void)
{
    Mousebinds_Cur = (MenuType_t)MENU_ID_MOUSEBINDS3;

    M_FillBackground();

    MN_DrTextACentered("ARTIFACTS", 10, cr[CR_YELLOW]);
    
    M_DrawBindButton(0, 20, mouseb_arti_boots, mouseb_arti_boots2);
    M_DrawBindButton(1, 30, mouseb_arti_torch, mouseb_arti_torch2);
    M_DrawBindButton(2, 40, mouseb_arti_krater, mouseb_arti_krater2);
    M_DrawBindButton(3, 50, mouseb_arti_incant, mouseb_arti_incant2);
    M_DrawBindButton(4, 60, mouseb_arti_all, mouseb_arti_all2);

    MN_DrTextACentered("RESET", 70, cr[CR_YELLOW]);

    M_DrawBindFooter("PAGE 3/3", true);
}

static void M_Bind_M_Boots (int choice)   { M_StartMouseBind(1200); } // mouseb_arti_boots
static void M_Bind_M_Torch (int choice)   { M_StartMouseBind(1201); } // mouseb_arti_torch
static void M_Bind_M_Krater (int choice)  { M_StartMouseBind(1202); } // mouseb_arti_krater
static void M_Bind_M_Incant (int choice)  { M_StartMouseBind(1203); } // mouseb_arti_incant
static void M_Bind_M_AllArti (int choice) { M_StartMouseBind(1204); } // mouseb_arti_all
...
static MenuItem_t ID_Menu_GamepadBinds[] = {
    { ITT_EFUNC, "GAMEPAD ENABLED",           M_ID_Gamepad_Enable,  0, MENU_NONE },
    { ITT_EMPTY, NULL,                        NULL,                 0, MENU_NONE },
    { ITT_EFUNC, "FIRE/ATTACK",               M_Bind_G_FireAttack,  0, MENU_NONE },
    { ITT_EFUNC, "STRAFE ON",                 M_Bind_G_StrafeOn,    0, MENU_NONE },
    { ITT_EFUNC, "USE",                       M_Bind_G_Use,         0, MENU_NONE },
    { ITT_EFUNC, "SPEED ON",                  M_Bind_G_SpeedOn,     0, MENU_NONE },
    { ITT_EFUNC, "JUMP",                      M_Bind_G_Jump,        0, MENU_NONE },
    { ITT_EFUNC, "STRAFE LEFT",               M_Bind_G_StrafeLeft,  0, MENU_NONE },
    { ITT_EFUNC, "STRAFE RIGHT",              M_Bind_G_StrafeRight, 0, MENU_NONE },
    { ITT_EFUNC, "PREV WEAPON",               M_Bind_G_PrevWeapon,  0, MENU_NONE },
    { ITT_EFUNC, "NEXT WEAPON",               M_Bind_G_NextWeapon,  0, MENU_NONE },
    { ITT_EFUNC, "ACTIVATE MENU",             M_Bind_G_Menu,        0, MENU_NONE },
    { ITT_EFUNC, "TOGGLE AUTOMAP",            M_Bind_G_Automap,     0, MENU_NONE },
};

static Menu_t ID_Def_GamepadBinds = {
    ID_MENU_CTRLSOFFSET, ID_MENU_TOPOFFSET,
    M_Draw_ID_GamepadBinds,
    ITEMCOUNT(ID_Menu_GamepadBinds), ID_Menu_GamepadBinds,
    0,
    SmallFont, false, true,
    MENU_ID_CONTROLS
};
...
static void M_DrawControllerName (void)
{
    ...
    byte *controller_color = (gamepad_enable && I_HasController()) ? cr[CR_GREEN_HX] : cr[CR_DARKRED];

    MN_DrTextACentered("DETECTED CONTROLLER:", 150, cr[CR_YELLOW]);
    MN_DrTextACentered(controller_name, 160, controller_color);
}
...
static void M_Draw_ID_GamepadBinds (void)
{
    ...
    MN_DrTextACentered("ENABLE GAMEPAD", 10, cr[CR_YELLOW]);

    sprintf(str, gamepad_enable ? "ON" : "OFF");
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 20,
                        gamepad_enable ? cr[CR_GREEN_HX] : cr[CR_RED],
                            gamepad_enable ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT],
                                LINE_ALPHA(0));

    MN_DrTextACentered("GAMEPAD BINDINGS", 30, cr[CR_YELLOW]);

    M_DrawBindGamepad(2, 40,  joybfire);
    M_DrawBindGamepad(3, 50,  joybstrafe);
    M_DrawBindGamepad(4, 60,  joybuse);
    M_DrawBindGamepad(5, 70,  joybspeed);
    M_DrawBindGamepad(6, 80,  joybjump);
    M_DrawBindGamepad(7, 90,  joybstrafeleft);
    M_DrawBindGamepad(8, 100,  joybstraferight);
    M_DrawBindGamepad(9, 110,  joybprevweapon);
    M_DrawBindGamepad(10, 120, joybnextweapon);
    M_DrawBindGamepad(11, 130, joybmenu);
    M_DrawBindGamepad(12, 140, joybautomap);
...
static void M_ID_Gamepad_Enable (int choice)   { gamepad_enable ^= 1;      }
static void M_Bind_G_FireAttack (int choice)   { M_StartGamepadBind(2000); }
static void M_Bind_G_StrafeOn (int choice)     { M_StartGamepadBind(2001); }
static void M_Bind_G_Use (int choice)          { M_StartGamepadBind(2002); }
static void M_Bind_G_SpeedOn (int choice)      { M_StartGamepadBind(2003); }
static void M_Bind_G_Jump (int choice)         { M_StartGamepadBind(2004); }
static void M_Bind_G_StrafeLeft (int choice)   { M_StartGamepadBind(2005); }
static void M_Bind_G_StrafeRight (int choice)  { M_StartGamepadBind(2006); }
static void M_Bind_G_PrevWeapon (int choice)   { M_StartGamepadBind(2007); }
static void M_Bind_G_NextWeapon (int choice)   { M_StartGamepadBind(2008); }
static void M_Bind_G_Menu (int choice)         { M_StartGamepadBind(2009); }
static void M_Bind_G_Automap (int choice)      { M_StartGamepadBind(2010); }

static MenuItem_t ID_Menu_GamepadBinds_2[] = {
    { ITT_EFUNC, "INVENTORY LEFT",             M_Bind_G_InventoryLeft,  0, MENU_NONE },
    { ITT_EFUNC, "INVENTORY RIGHT",            M_Bind_G_InventoryRight, 0, MENU_NONE },
    { ITT_EFUNC, "USE ARTIFACT",               M_Bind_G_UseArtifact,    0, MENU_NONE },
    { ITT_EFUNC, "FLY UP",                     M_Bind_G_FlyUp,          0, MENU_NONE },
    { ITT_EFUNC, "FLY DOWN",                   M_Bind_G_FlyDown,        0, MENU_NONE },
    { ITT_EFUNC, "FLY CENTER",                 M_Bind_G_FlyCenter,      0, MENU_NONE },
    { ITT_EMPTY, NULL,                         NULL,                    0, MENU_NONE },
    { ITT_EFUNC, "RESET BINDINGS TO DEFAULT",  M_Bind_G_Reset,          0, MENU_NONE },
};

static Menu_t ID_Def_GamepadBinds_2 = {
    ID_MENU_CTRLSOFFSET, ID_MENU_TOPOFFSET,
    M_Draw_ID_GamepadBinds_2,
    ITEMCOUNT(ID_Menu_GamepadBinds_2), ID_Menu_GamepadBinds_2,
    0,
    SmallFont, false, true,
    MENU_ID_CONTROLS
};
...
static void M_Draw_ID_GamepadBinds_2 (void)
{
    GamepadBinds_Cur = (MenuType_t)MENU_ID_GAMEPADBINDS2;
    M_FillBackground();

    MN_DrTextACentered("GAMEPAD BINDINGS", 10, cr[CR_YELLOW]);

    M_DrawBindGamepad(0, 20, joybinvleft);
    M_DrawBindGamepad(1, 30, joybinvright);
    M_DrawBindGamepad(2, 40, joybuseartifact);
    M_DrawBindGamepad(3, 50, joybflyup);
    M_DrawBindGamepad(4, 60, joybflydown);
    M_DrawBindGamepad(5, 70, joybflycenter);
...
static void M_Bind_G_InventoryLeft (int choice)  { M_StartGamepadBind(2011); }
static void M_Bind_G_InventoryRight (int choice) { M_StartGamepadBind(2012); }
static void M_Bind_G_UseArtifact (int choice)    { M_StartGamepadBind(2013); }
static void M_Bind_G_FlyUp (int choice)          { M_StartGamepadBind(2014); }
static void M_Bind_G_FlyDown (int choice)        { M_StartGamepadBind(2015); }
static void M_Bind_G_FlyCenter (int choice)      { M_StartGamepadBind(2016); }

static void M_Bind_G_Reset (int choice)
{
    MenuActive = false;
    askforquit = true;
    typeofask = 11;      // [PN] gamepad binds reset
}
...
static MenuItem_t ID_Menu_GamepadSettings_1[] = {
    { ITT_LRFUNC2, "ANALOG MOVEMENT",  M_ID_Gamepad_UseAnalog,        0, MENU_NONE },
    { ITT_LRFUNC1, "TURN SENSITIVITY", M_ID_Gamepad_TurnSensitivity,  0, MENU_NONE },
    { ITT_LRFUNC1, "MOVE SENSITIVITY", M_ID_Gamepad_MoveSensitivity,  0, MENU_NONE },
    { ITT_LRFUNC1, "LOOK SENSITIVITY", M_ID_Gamepad_LookSensitivity,  0, MENU_NONE },
    { ITT_LRFUNC1, "X AXIS",           M_ID_Gamepad_AxisX,            0, MENU_NONE },
    { ITT_LRFUNC1, "Y AXIS",           M_ID_Gamepad_AxisY,            0, MENU_NONE },
    { ITT_LRFUNC1, "STRAFE AXIS",      M_ID_Gamepad_AxisStrafe,       0, MENU_NONE },
    { ITT_LRFUNC1, "LOOK AXIS",        M_ID_Gamepad_AxisLook,         0, MENU_NONE },
    { ITT_EMPTY,   NULL,               NULL,                          0, MENU_NONE },
};
...
static Menu_t ID_Def_GamepadSettings_1 = {
    ...
    ITEMCOUNT(ID_Menu_GamepadSettings_1), ID_Menu_GamepadSettings_1,
    0,
    SmallFont, false, true,
    MENU_ID_CONTROLS
};
...
static MenuItem_t ID_Menu_GamepadSettings_2[] = {
    { ITT_LRFUNC2, "X AXIS INVERT",       M_ID_Gamepad_InvertX,          0, MENU_NONE },
    { ITT_LRFUNC2, "Y AXIS INVERT",       M_ID_Gamepad_InvertY,          0, MENU_NONE },
    { ITT_LRFUNC2, "STRAFE AXIS INVERT",  M_ID_Gamepad_InvertStrafe,     0, MENU_NONE },
    { ITT_LRFUNC2, "LOOK AXIS INVERT",    M_ID_Gamepad_InvertLook,       0, MENU_NONE },
    { ITT_LRFUNC1, "X DEAD ZONE",         M_ID_Gamepad_DeadZoneX,        0, MENU_NONE },
    { ITT_LRFUNC1, "Y DEAD ZONE",         M_ID_Gamepad_DeadZoneY,        0, MENU_NONE },
    { ITT_LRFUNC1, "STRAFE DEAD ZONE",    M_ID_Gamepad_DeadZoneStrafe,   0, MENU_NONE },
    { ITT_LRFUNC1, "LOOK DEAD ZONE",      M_ID_Gamepad_DeadZoneLook,     0, MENU_NONE },
    { ITT_EMPTY,   NULL,                  NULL,                          0, MENU_NONE },
};
...
static Menu_t ID_Def_GamepadSettings_2 = {
    ...
    ITEMCOUNT(ID_Menu_GamepadSettings_2), ID_Menu_GamepadSettings_2,
    0,
    SmallFont, false, true,
    MENU_ID_CONTROLS
};
...
static void M_Draw_ID_GamepadSettings_1 (void)
{
    ...
    GamepadBinds_Cur = (MenuType_t)MENU_ID_GAMEPADSETTINGS1;
    M_FillBackground();

    MN_DrTextACentered("GAMEPAD SETTINGS", 10, cr[CR_YELLOW]);
    MN_DrTextACentered("PRESS ENTER/LEFT/RIGHT TO CHANGE", 170, cr[CR_GRAY]);

    MN_DrTextAGlow(use_analog ? "ON" : "OFF", M_ItemRightAlign(use_analog ? "ON" : "OFF"), 20,
                   use_analog ? cr[CR_GREEN_HX] : cr[CR_RED],
                   use_analog ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT], LINE_ALPHA(0));

    M_snprintf(str, sizeof(str), "%d", joystick_turn_sensitivity);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 30, cr[CR_GREEN_HX], cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(1));
    M_snprintf(str, sizeof(str), "%d", joystick_move_sensitivity);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 40, cr[CR_GREEN_HX], cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(2));
    M_snprintf(str, sizeof(str), "%d", joystick_look_sensitivity);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 50, cr[CR_GREEN_HX], cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(3));

    MN_DrTextAGlow(M_GamepadAxisName(joystick_x_axis, str, sizeof(str)), M_ItemRightAlign(M_GamepadAxisName(joystick_x_axis, str, sizeof(str))), 60,
                   joystick_x_axis < 0 ? cr[CR_RED] : cr[CR_GREEN_HX],
                   joystick_x_axis < 0 ? cr[CR_RED_BRIGHT] : cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(4));
    MN_DrTextAGlow(M_GamepadAxisName(joystick_y_axis, str, sizeof(str)), M_ItemRightAlign(M_GamepadAxisName(joystick_y_axis, str, sizeof(str))), 70,
                   joystick_y_axis < 0 ? cr[CR_RED] : cr[CR_GREEN_HX],
                   joystick_y_axis < 0 ? cr[CR_RED_BRIGHT] : cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(5));
    MN_DrTextAGlow(M_GamepadAxisName(joystick_strafe_axis, str, sizeof(str)), M_ItemRightAlign(M_GamepadAxisName(joystick_strafe_axis, str, sizeof(str))), 80,
                   joystick_strafe_axis < 0 ? cr[CR_RED] : cr[CR_GREEN_HX],
                   joystick_strafe_axis < 0 ? cr[CR_RED_BRIGHT] : cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(6));
    MN_DrTextAGlow(M_GamepadAxisName(joystick_look_axis, str, sizeof(str)), M_ItemRightAlign(M_GamepadAxisName(joystick_look_axis, str, sizeof(str))), 90,
                   joystick_look_axis < 0 ? cr[CR_RED] : cr[CR_GREEN_HX],
                   joystick_look_axis < 0 ? cr[CR_RED_BRIGHT] : cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(7));
...
static void M_Draw_ID_GamepadSettings_2 (void)
{
    ...
    GamepadBinds_Cur = (MenuType_t)MENU_ID_GAMEPADSETTINGS2;
    M_FillBackground();

    MN_DrTextACentered("GAMEPAD SETTINGS", 10, cr[CR_YELLOW]);
    MN_DrTextACentered("PRESS ENTER/LEFT/RIGHT TO CHANGE", 170, cr[CR_GRAY]);

    MN_DrTextAGlow(joystick_x_invert ? "ON" : "OFF", M_ItemRightAlign(joystick_x_invert ? "ON" : "OFF"), 20,
                   joystick_x_invert ? cr[CR_GREEN_HX] : cr[CR_RED],
                   joystick_x_invert ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT], LINE_ALPHA(0));
    MN_DrTextAGlow(joystick_y_invert ? "ON" : "OFF", M_ItemRightAlign(joystick_y_invert ? "ON" : "OFF"), 30,
                   joystick_y_invert ? cr[CR_GREEN_HX] : cr[CR_RED],
                   joystick_y_invert ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT], LINE_ALPHA(1));
    MN_DrTextAGlow(joystick_strafe_invert ? "ON" : "OFF", M_ItemRightAlign(joystick_strafe_invert ? "ON" : "OFF"), 40,
                   joystick_strafe_invert ? cr[CR_GREEN_HX] : cr[CR_RED],
                   joystick_strafe_invert ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT], LINE_ALPHA(2));
    MN_DrTextAGlow(joystick_look_invert ? "ON" : "OFF", M_ItemRightAlign(joystick_look_invert ? "ON" : "OFF"), 50,
                   joystick_look_invert ? cr[CR_GREEN_HX] : cr[CR_RED],
                   joystick_look_invert ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT], LINE_ALPHA(3));

    M_snprintf(str, sizeof(str), "%d", joystick_x_dead_zone);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 60, cr[CR_GREEN_HX], cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(4));
    M_snprintf(str, sizeof(str), "%d", joystick_y_dead_zone);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 70, cr[CR_GREEN_HX], cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(5));
    M_snprintf(str, sizeof(str), "%d", joystick_strafe_dead_zone);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 80, cr[CR_GREEN_HX], cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(6));
    M_snprintf(str, sizeof(str), "%d", joystick_look_dead_zone);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 90, cr[CR_GREEN_HX], cr[CR_GREEN_HX_BRIGHT], LINE_ALPHA(7));

    M_DrawControllerName();
    M_DrawGamepadPagesFooter("4");
}
...
static void M_Draw_ID_Widgets (void)
{
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 20,
                        widget_font ? cr[CR_GREEN_HX] : cr[CR_DARKRED], 
                            widget_font ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT], 
                                LINE_ALPHA(0));
...
static MenuItem_t ID_Menu_Gameplay_1[] = {
    ...
    { ITT_LRFUNC1, "SPRITE SHADOWS",            M_ID_SpriteShadows,   0, MENU_NONE },
...
static void M_Draw_ID_Gameplay_1 (void)
{
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 50,
                        vis_sprite_shadows ? cr[CR_GREEN_HX] : cr[CR_DARKRED],
                            vis_sprite_shadows ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT],
                                LINE_ALPHA(3));
...
static void M_Draw_ID_Gameplay_3 (void)
{
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 100,
                        demo_internal ? cr[CR_DARKRED] : cr[CR_GREEN_HX],
                            demo_internal ?  cr[CR_RED_BRIGHT] : cr[CR_GREEN_HX_BRIGHT],
                                LINE_ALPHA(8));
...
static void M_Draw_ID_Misc_1 (void)
{
...
#if defined(_WIN32) && !defined(_WIN32_WCE)
    MN_DrTextACentered("LAUNCHER", 130, cr[CR_YELLOW]);

    // Cap framerate in the menu
    sprintf(str, show_startup_launcher ? "ON" : "OFF" );
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 140,
                        show_startup_launcher ? cr[CR_GREEN_HX] : cr[CR_DARKRED],
                            show_startup_launcher ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT],
                                LINE_ALPHA(12));
#endif
    ...
    if (CurrentItPos == 4)
    {
        ...
        MN_DrTextACentered(colorblind_hint[a11y_colorblind], 150, cr[CR_LIGHTGRAY_DARK]);
    }
    ...
    if (CurrentItPos == 6)
    {
        ...
        {
            case 1:
                MN_DrTextACentered(first_line, 150, cr[CR_LIGHTGRAY_DARK]);
                MN_DrTextACentered(second_line1, 160, cr[CR_LIGHTGRAY_DARK]);
                break;

            case 2:
                MN_DrTextACentered(first_line, 150, cr[CR_LIGHTGRAY_DARK]);
                MN_DrTextACentered(second_line2, 160, cr[CR_LIGHTGRAY_DARK]);
                break;

            default:
                MN_DrTextACentered(off, 150, cr[CR_LIGHTGRAY_DARK]);
                break;            
        }
    }
    else
    {
        // < Scroll pages >
        M_DrawScrollPages(ID_MENU_LEFTOFFSET_BIG, 150, 13, "1/2");
    }
}
...
static void M_Draw_ID_Misc_2 (void)
{
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 20,
                        rewind_enable ? cr[CR_GREEN_HX] : cr[CR_DARKRED],
                            rewind_enable ? cr[CR_GREEN_HX_BRIGHT] : cr[CR_RED_BRIGHT],
                                LINE_ALPHA(0));
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 30,
                       !rewind_enable ? cr[CR_DARKRED] :
                        rewind_interval == 600 ? cr[CR_YELLOW] : cr[CR_GREEN_HX],
                           !rewind_enable ? cr[CR_RED_BRIGHT] : 
                            rewind_interval == 600 ? cr[CR_YELLOW_BRIGHT] : cr[CR_GREEN_HX_BRIGHT],
                                LINE_ALPHA(1));
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 40,
                       !rewind_enable ? cr[CR_DARKRED] :
                        rewind_depth == 600 ? cr[CR_YELLOW] : cr[CR_GREEN_HX],
                           !rewind_enable ? cr[CR_RED_BRIGHT] :
                            rewind_depth == 600 ? cr[CR_YELLOW_BRIGHT] : cr[CR_GREEN_HX_BRIGHT],
                                LINE_ALPHA(2));
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 50,
                       !rewind_enable ? cr[CR_DARKRED] :
                        rewind_timeout == 25 ? cr[CR_YELLOW] : cr[CR_GREEN_HX],
                           !rewind_enable ? cr[CR_RED_BRIGHT] :
                            rewind_timeout == 25 ? cr[CR_YELLOW_BRIGHT] : cr[CR_GREEN_HX_BRIGHT],
                                LINE_ALPHA(3));
...
static Menu_t *Menus[] = {
    ...
    &ID_Def_MouseBinds3,
    &ID_Def_GamepadBinds,
    &ID_Def_GamepadBinds_2,
    &ID_Def_GamepadSettings_1,
    &ID_Def_GamepadSettings_2,
    &ID_Def_Widgets,
    &ID_Def_Automap,
    &ID_Def_Gameplay_1,
    &ID_Def_Gameplay_2,
...
// -----------------------------------------------------------------------------
// MN_DrTextAFade
//  [PN] Write a string using FontAYellow with custom alpha fade.
// -----------------------------------------------------------------------------
...
static const char *const QuitEndMsg[] = {
    ...
    "RESET GAMEPAD BINDINGS TO DEFAULT VALUES?",   // [PN] typeofask 11 (reset gamepad binds)
};
...
void MN_Drawer(void)
{
    ...
    if (MenuActive == false)
    {
        ...
        if (askforquit)
        {
            ...
            if (typeofask == 7 || typeofask == 8 || typeofask == 11)
            {
                M_FillBackground();
...
static boolean MN_ID_TypeOfAsk (void)
{
    switch (typeofask)
    {
        ...
        case 11: // [PN] Reset gamepad binds.
            M_ResetGamepadBinds();
            if (!netgame && !demoplayback)
            {
                paused = true;
...
boolean MN_Responder(event_t * event)
{
    ...
    {
        const unsigned int pressed = (unsigned int) event->data1;
...
#define JOY_BUTTON_PRESSED(x) (JOY_BUTTON_MAPPED(x) && (newly_pressed & ~joybuttons_blocked & (1u << (x))) != 0)

        if (JOY_BUTTON_PRESSED(joybfire))
        {
            key = askforquit ? key_menu_confirm : key_menu_forward;
            joywait = I_GetTime() + 5;
        }

        if (JOY_BUTTON_PRESSED(joybuse))
        {
            key = askforquit ? key_menu_abort : key_menu_back;
            joywait = I_GetTime() + 5;
        }

        if (joybmenu >= 0 && (newly_pressed & ~joybuttons_blocked & (1u << joybmenu)) != 0)
        {
            joybuttons_prev = pressed;
            joywait = I_GetTime() + 5;
            MN_ActivateMenu();
...
#undef JOY_BUTTON_PRESSED
#undef JOY_BUTTON_MAPPED
    ...
    else
    {
        ...
        if (event->type == ev_mouse && mousewait < I_GetTime())
        {
            ...
            */
            ...
            {
                if (CurrentItPos == -1
                || (CurrentMenu->ScrollAR && !FileMenuKeySteal && !KbdIsBinding && !GamepadIsBinding))
                {
                    M_ScrollPages(1);
                }
                else
    ...
    {
        R_SetViewSize(13, dp_detail_level);
        S_StartSound(NULL, SFX_PICKUP_ITEM);
    ...
    if (askforquit)
    {
        ...
        || ((key == key_menu_quit || key == key_menu_quit2) && typeofask == 1))
        {
            MN_ID_TypeOfAsk();
        }
        else
        ...
        || (event->type == ev_mouse && event->data1 & 2))  // [JN] Cancel by right mouse button.
        {
            ...
            if (typeofask == 7 || typeofask == 8 || typeofask == 9 || typeofask == 11)
            {
                if (!netgame && !demoplayback)
                {
                    paused = true;
    ...
    if (!MenuActive && !chatmodeon)
    {
        ...
        // [JN] In the original released source, the game
        // also checks for // "DevMaps == false" in the first condition.
        // It’s supposed to be enabled with the "-devmaps" command-line parameter,
        // and the F12 key should no longer be hardcoded.
        /*
    ...
    ||  key == key_menu_gamma  || key == key_menu_gamma2)         // F11 (gamma correction)
    {
    ...
    if (!FileMenuKeySteal)
    {
        ...
        else if (key == key_menu_del)
        {
            ...
            else if (CurrentMenu == &ID_Def_MouseBinds1
                  || CurrentMenu == &ID_Def_MouseBinds2
                  || CurrentMenu == &ID_Def_MouseBinds3)
            {
                M_ClearMouseBind(CurrentItPos);
            }
            else if (CurrentMenu == &ID_Def_GamepadBinds
                  || CurrentMenu == &ID_Def_GamepadBinds_2)
            {
                M_ClearGamepadBind(CurrentItPos);
            }
            return (true);
        }
...
{
    if (CurrentItPos == CurrentItPosOn && (KbdIsBinding || MouseIsBinding || GamepadIsBinding))
    {
...
static void M_DrawBindFooter (char *pagenum, boolean drawPages)
{
    ...
    if (drawPages)
    {
        ...
        MN_DrTextACentered(M_StringJoin(pagenum, NULL), 180, cr[CR_GRAY]);
...
//  Draw footer for gamepad bindings pages with 4-page numeration.
// -----------------------------------------------------------------------------

static void M_DrawGamepadPagesFooter (const char *pagenum)
{
    MN_DrTextA("PGUP", ID_MENU_LEFTOFFSET, 180, cr[CR_GRAY]);
    MN_DrTextACentered(M_StringJoin("PAGE ", pagenum, "/4", NULL), 180, cr[CR_GRAY]);
    MN_DrTextA("PGDN", M_ItemRightAlign("PGDN"), 180, cr[CR_GRAY]);
}
...
static const KeyBindEntry_t keybinds[] =
{
    ...
    KEYBIND_ENTRY(106, &ID_Def_Keybinds_1, 6,  key_strafe,      key_strafe2,      KEY_RALT,       0, KBS_GLOBAL),
    KEYBIND_ENTRY(107, &ID_Def_Keybinds_1, 7,  key_speed,       key_speed2,       KEY_RSHIFT,     0, KBS_GLOBAL),
    KEYBIND_ENTRY(108, &ID_Def_Keybinds_1, 8,  key_jump,        key_jump2,        '/',            0, KBS_GLOBAL),
    KEYBIND_ENTRY(109, &ID_Def_Keybinds_1, 9,  key_180turn,     key_180turn2,     0,              0, KBS_GLOBAL),
    KEYBIND_ENTRY(110, &ID_Def_Keybinds_1, 11, key_fire,        key_fire2,        KEY_RCTRL,      0, KBS_GLOBAL),
    KEYBIND_ENTRY(111, &ID_Def_Keybinds_1, 12, key_use,         key_use2,         ' ',            0, KBS_GLOBAL),

    // Page 2
    KEYBIND_ENTRY(200, &ID_Def_Keybinds_2, 0,  key_lookup,      key_lookup2,      KEY_PGDN,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(201, &ID_Def_Keybinds_2, 1,  key_lookdown,    key_lookdown2,    KEY_DEL,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(202, &ID_Def_Keybinds_2, 2,  key_lookcenter,  key_lookcenter2,  KEY_END,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(203, &ID_Def_Keybinds_2, 4,  key_flyup,       key_flyup2,       KEY_PGUP,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(204, &ID_Def_Keybinds_2, 5,  key_flydown,     key_flydown2,     KEY_INS,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(205, &ID_Def_Keybinds_2, 6,  key_flycenter,   key_flycenter2,   KEY_HOME,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(206, &ID_Def_Keybinds_2, 8,  key_invleft,     key_invleft2,     '[',       0, KBS_GLOBAL),
    KEYBIND_ENTRY(207, &ID_Def_Keybinds_2, 9,  key_invright,    key_invright2,    ']',       0, KBS_GLOBAL),
    KEYBIND_ENTRY(208, &ID_Def_Keybinds_2, 10, key_useartifact, key_useartifact2, KEY_ENTER, 0, KBS_GLOBAL),

    // Page 3
    KEYBIND_ENTRY(300, &ID_Def_Keybinds_3, 0,  key_autorun,       key_autorun2,       KEY_CAPSLOCK, 0, KBS_GLOBAL),
    KEYBIND_ENTRY(301, &ID_Def_Keybinds_3, 1,  key_mouse_look,    key_mouse_look2,    0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(302, &ID_Def_Keybinds_3, 2,  key_novert,        key_novert2,        0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(303, &ID_Def_Keybinds_3, 4,  key_prevlevel,     key_prevlevel2,     0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(304, &ID_Def_Keybinds_3, 5,  key_reloadlevel,   key_reloadlevel2,   0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(305, &ID_Def_Keybinds_3, 6,  key_nextlevel,     key_nextlevel2,     0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(306, &ID_Def_Keybinds_3, 7,  key_demospeed,     key_demospeed2,     0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(307, &ID_Def_Keybinds_3, 8,  key_flip_levels,   key_flip_levels2,   0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(308, &ID_Def_Keybinds_3, 9,  key_widget_enable, key_widget_enable2, 0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(309, &ID_Def_Keybinds_3, 11, key_spectator,     key_spectator2,     0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(310, &ID_Def_Keybinds_3, 12, key_freeze,        key_freeze2,        0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(311, &ID_Def_Keybinds_3, 13, key_notarget,      key_notarget2,      0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(312, &ID_Def_Keybinds_3, 14, key_buddha,        key_buddha2,        0,            0, KBS_GLOBAL),

    // Page 4
    KEYBIND_ENTRY(400, &ID_Def_Keybinds_4, 0,  key_shorttics,             key_shorttics2,            0,    0, KBS_GLOBAL),
    KEYBIND_ENTRY(401, &ID_Def_Keybinds_4, 1,  key_speed_up,    key_speed_up2,    0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(402, &ID_Def_Keybinds_4, 2,  key_speed_down,  key_speed_down2,  0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(403, &ID_Def_Keybinds_4, 3,  key_speed_reset, key_speed_reset2, 0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(404, &ID_Def_Keybinds_4, 4,  key_rewind,                key_rewind2,               0,    0, KBS_GLOBAL),
    KEYBIND_ENTRY(405, &ID_Def_Keybinds_4, 6,  key_weapon1,               key_weapon1_2,             '1',  0, KBS_GLOBAL),
    KEYBIND_ENTRY(406, &ID_Def_Keybinds_4, 7,  key_weapon2,               key_weapon2_2,             '2',  0, KBS_GLOBAL),
    KEYBIND_ENTRY(407, &ID_Def_Keybinds_4, 8,  key_weapon3,               key_weapon3_2,             '3',  0, KBS_GLOBAL),
    KEYBIND_ENTRY(408, &ID_Def_Keybinds_4, 9,  key_weapon4,               key_weapon4_2,             '4',  0, KBS_GLOBAL),
    KEYBIND_ENTRY(409, &ID_Def_Keybinds_4, 10, key_prevweapon,            key_prevweapon2,           0,    0, KBS_GLOBAL),
    KEYBIND_ENTRY(410, &ID_Def_Keybinds_4, 11, key_nextweapon,            key_nextweapon2,           0,    0, KBS_GLOBAL),
    KEYBIND_ENTRY(411, &ID_Def_Keybinds_4, 13, key_arti_health,           key_arti_health2,          '\\', 0, KBS_GLOBAL),
    KEYBIND_ENTRY(412, &ID_Def_Keybinds_4, 14, key_arti_urn,              key_arti_urn2,             0,    0, KBS_GLOBAL),

    // Page 5
    KEYBIND_ENTRY(500, &ID_Def_Keybinds_5, 0,  key_arti_poisonbag,        key_arti_poisonbag2,       '0',  0, KBS_GLOBAL),
    KEYBIND_ENTRY(501, &ID_Def_Keybinds_5, 1,  key_arti_blastradius,      key_arti_blastradius2,     '9',  0, KBS_GLOBAL),
    KEYBIND_ENTRY(502, &ID_Def_Keybinds_5, 2,  key_arti_invulnerability,  key_arti_invulnerability2, '5',  0, KBS_GLOBAL),
    KEYBIND_ENTRY(503, &ID_Def_Keybinds_5, 3,  key_arti_egg,           key_arti_egg2,           '6',           0, KBS_GLOBAL),
    KEYBIND_ENTRY(504, &ID_Def_Keybinds_5, 4,  key_arti_teleport,      key_arti_teleport2,      '8',           0, KBS_GLOBAL),
    KEYBIND_ENTRY(505, &ID_Def_Keybinds_5, 5,  key_arti_teleportother, key_arti_teleportother2, '7',           0, KBS_GLOBAL),
    KEYBIND_ENTRY(506, &ID_Def_Keybinds_5, 6,  key_arti_wings,         key_arti_wings2,         0,             0, KBS_GLOBAL),
    KEYBIND_ENTRY(507, &ID_Def_Keybinds_5, 7,  key_arti_servant,       key_arti_servant2,       0,             0, KBS_GLOBAL),
    KEYBIND_ENTRY(508, &ID_Def_Keybinds_5, 8,  key_arti_bracers,       key_arti_bracers2,       0,             0, KBS_GLOBAL),
    KEYBIND_ENTRY(509, &ID_Def_Keybinds_5, 9,  key_arti_boots,         key_arti_boots2,         0,             0, KBS_GLOBAL),
    KEYBIND_ENTRY(510, &ID_Def_Keybinds_5, 10, key_arti_torch,         key_arti_torch2,         0,             0, KBS_GLOBAL),
    KEYBIND_ENTRY(511, &ID_Def_Keybinds_5, 11, key_arti_krater,        key_arti_krater2,        0,             0, KBS_GLOBAL),
    KEYBIND_ENTRY(512, &ID_Def_Keybinds_5, 12, key_arti_incant,        key_arti_incant2,        0,             0, KBS_GLOBAL),
    KEYBIND_ENTRY(513, &ID_Def_Keybinds_5, 13, key_arti_all,           key_arti_all2,           KEY_BACKSPACE, 0, KBS_GLOBAL),

    // Page 6
    KEYBIND_ENTRY(600, &ID_Def_Keybinds_6, 0,  key_map_toggle,    key_map_toggle2,    KEY_TAB,      0, KBS_GLOBAL),
    KEYBIND_ENTRY(601, &ID_Def_Keybinds_6, 1,  key_map_mini,      key_map_mini2,      0,            0, KBS_GLOBAL),
    KEYBIND_ENTRY(602, &ID_Def_Keybinds_6, 2,  key_map_zoomin,    key_map_zoomin2,    '=',  KEYP_PLUS, KBS_AUTOMAP_ONLY),
    KEYBIND_ENTRY(603, &ID_Def_Keybinds_6, 3,  key_map_zoomout,   key_map_zoomout2,   '-', KEYP_MINUS, KBS_AUTOMAP_ONLY),
    KEYBIND_ENTRY(604, &ID_Def_Keybinds_6, 4,  key_map_maxzoom,   key_map_maxzoom2,   '0',          0, KBS_AUTOMAP_ONLY),
    KEYBIND_ENTRY(605, &ID_Def_Keybinds_6, 5,  key_map_follow,    key_map_follow2,    'f',          0, KBS_AUTOMAP_ONLY),
    KEYBIND_ENTRY(606, &ID_Def_Keybinds_6, 6,  key_map_rotate,    key_map_rotate2,    'r',          0, KBS_AUTOMAP_ONLY),
    KEYBIND_ENTRY(607, &ID_Def_Keybinds_6, 7,  key_map_overlay,   key_map_overlay2,   'o',          0, KBS_AUTOMAP_ONLY),
    KEYBIND_ENTRY(608, &ID_Def_Keybinds_6, 8,  key_map_mousepan,  key_map_mousepan2,  0,            0, KBS_AUTOMAP_ONLY),
    KEYBIND_ENTRY(609, &ID_Def_Keybinds_6, 9,  key_map_grid,      key_map_grid2,      'g',          0, KBS_AUTOMAP_ONLY),
    KEYBIND_ENTRY(610, &ID_Def_Keybinds_6, 10, key_map_mark,      key_map_mark2,      'm',          0, KBS_AUTOMAP_ONLY),
    KEYBIND_ENTRY(611, &ID_Def_Keybinds_6, 11, key_map_clearmark, key_map_clearmark2, 'c',          0, KBS_AUTOMAP_ONLY),

    // Page 7
    KEYBIND_ENTRY(700, &ID_Def_Keybinds_7, 0,  key_menu_help,     key_menu_help2,     KEY_F1,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(701, &ID_Def_Keybinds_7, 1,  key_menu_save,     key_menu_save2,     KEY_F2,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(702, &ID_Def_Keybinds_7, 2,  key_menu_load,     key_menu_load2,     KEY_F3,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(703, &ID_Def_Keybinds_7, 3,  key_menu_volume,   key_menu_volume2,   KEY_F4,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(704, &ID_Def_Keybinds_7, 4,  key_menu_detail,   key_menu_detail2,   KEY_F5,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(705, &ID_Def_Keybinds_7, 5,  key_menu_qsave,    key_menu_qsave2,    KEY_F6,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(706, &ID_Def_Keybinds_7, 6,  key_menu_endgame,  key_menu_endgame2,  KEY_F7,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(707, &ID_Def_Keybinds_7, 7,  key_menu_messages, key_menu_messages2, KEY_F8,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(708, &ID_Def_Keybinds_7, 8,  key_menu_qload,    key_menu_qload2,    KEY_F9,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(709, &ID_Def_Keybinds_7, 9,  key_menu_quit,     key_menu_quit2,     KEY_F10, 0, KBS_GLOBAL),
    KEYBIND_ENTRY(710, &ID_Def_Keybinds_7, 10, key_menu_gammad,   key_menu_gammad2,   0,       0, KBS_GLOBAL),
    KEYBIND_ENTRY(711, &ID_Def_Keybinds_7, 11, key_menu_gamma,    key_menu_gamma2,    KEY_F11, 0, KBS_GLOBAL),
    KEYBIND_ENTRY(712, &ID_Def_Keybinds_7, 12, key_spy,           key_spy2,           KEY_F12, 0, KBS_GLOBAL),

    // Page 8
    KEYBIND_ENTRY(800, &ID_Def_Keybinds_8, 0,  key_pause,              key_pause2,              KEY_PAUSE,  0, KBS_GLOBAL),
    KEYBIND_ENTRY(801, &ID_Def_Keybinds_8, 1,  key_menu_screenshot,    key_menu_screenshot2,    KEY_PRTSCR, 0, KBS_GLOBAL),
    KEYBIND_ENTRY(802, &ID_Def_Keybinds_8, 2,  key_menu_cleanshot,     key_menu_cleanshot2,     0,          0, KBS_GLOBAL),
    KEYBIND_ENTRY(803, &ID_Def_Keybinds_8, 3,  key_message_refresh_hr, key_message_refresh_hr2, 0,          0, KBS_GLOBAL),
    KEYBIND_ENTRY(804, &ID_Def_Keybinds_8, 4,  key_demo_quit,          key_demo_quit2,          'q',        0, KBS_GLOBAL),
    KEYBIND_ENTRY(805, &ID_Def_Keybinds_8, 5,  key_switch_ost,         key_switch_ost2,         0,          0, KBS_GLOBAL),
    KEYBIND_ENTRY(806, &ID_Def_Keybinds_8, 7,  key_multi_msg,          key_multi_msg2,          't',        0, KBS_GLOBAL),
    /*
    KEYBIND_ENTRY(807, &ID_Def_Keybinds_8, 7,  key_multi_msgplayer[0], key_multi_msgplayer2[0], 'g',        0, KBS_SENDTO_ONLY),
    KEYBIND_ENTRY(808, &ID_Def_Keybinds_8, 8,  key_multi_msgplayer[1], key_multi_msgplayer2[1], 'i',        0, KBS_SENDTO_ONLY),
    KEYBIND_ENTRY(809, &ID_Def_Keybinds_8, 9,  key_multi_msgplayer[2], key_multi_msgplayer2[2], 'b',        0, KBS_SENDTO_ONLY),
    KEYBIND_ENTRY(810, &ID_Def_Keybinds_8, 10, key_multi_msgplayer[3], key_multi_msgplayer2[3], 'r',        0, KBS_SENDTO_ONLY),
    */
};
...
static void M_DrawBindKey (int itemNum, int yPos, int key1, int key2)
{
    ...
    MN_DrTextAGlow(text, M_ItemRightAlign(text), yPos,
        CurrentItPos == itemNum && KbdIsBinding ? cr[CR_YELLOW] : (empty ? cr[CR_RED] : cr[CR_GREEN_HX]),
        CurrentItPos == itemNum && KbdIsBinding ? cr[CR_YELLOW_BRIGHT] : (empty ? cr[CR_RED_BRIGHT] : cr[CR_GREEN_HX_BRIGHT]),
        LINE_ALPHA(itemNum));
}
...
{
    MOUSEBIND_ENTRY(1000, &ID_Def_MouseBinds1, 0,  mousebfire,        mousebfire2,         0, -1),
    MOUSEBIND_ENTRY(1001, &ID_Def_MouseBinds1, 1,  mousebforward,     mousebforward2,      2, -1),
    MOUSEBIND_ENTRY(1002, &ID_Def_MouseBinds1, 2,  mousebbackward,    mousebbackward2,    -1, -1),
    MOUSEBIND_ENTRY(1003, &ID_Def_MouseBinds1, 3,  mousebuse,         mousebuse2,         -1, -1),
    MOUSEBIND_ENTRY(1004, &ID_Def_MouseBinds1, 4,  mousebjump,        mousebjump2,        -1, -1),
    MOUSEBIND_ENTRY(1005, &ID_Def_MouseBinds1, 5,  mousebspeed,       mousebspeed2,       -1, -1),
    MOUSEBIND_ENTRY(1006, &ID_Def_MouseBinds1, 6,  mousebstrafe,      mousebstrafe2,       1, -1),
    MOUSEBIND_ENTRY(1007, &ID_Def_MouseBinds1, 7,  mousebstrafeleft,  mousebstrafeleft2,  -1, -1),
    MOUSEBIND_ENTRY(1008, &ID_Def_MouseBinds1, 8,  mousebstraferight, mousebstraferight2, -1, -1),
    MOUSEBIND_ENTRY(1009, &ID_Def_MouseBinds1, 9,  mousebprevweapon,  mousebprevweapon2,   4, -1),
    MOUSEBIND_ENTRY(1010, &ID_Def_MouseBinds1, 10, mousebnextweapon,  mousebnextweapon2,   3, -1),

    MOUSEBIND_ENTRY(1100, &ID_Def_MouseBinds2, 0,  mousebinvleft,               mousebinvleft2,               -1, -1),
    MOUSEBIND_ENTRY(1101, &ID_Def_MouseBinds2, 1,  mousebinvright,              mousebinvright2,              -1, -1),
    MOUSEBIND_ENTRY(1102, &ID_Def_MouseBinds2, 2,  mousebuseartifact,           mousebuseartifact2,           -1, -1),
    MOUSEBIND_ENTRY(1103, &ID_Def_MouseBinds2, 3,  mouseb_arti_health,          mouseb_arti_health2,          -1, -1),
    MOUSEBIND_ENTRY(1104, &ID_Def_MouseBinds2, 4,  mouseb_arti_urn,             mouseb_arti_urn2,             -1, -1),
    MOUSEBIND_ENTRY(1105, &ID_Def_MouseBinds2, 5,  mouseb_arti_poisonbag,       mouseb_arti_poisonbag2,       -1, -1),
    MOUSEBIND_ENTRY(1106, &ID_Def_MouseBinds2, 6,  mouseb_arti_blastradius,     mouseb_arti_blastradius2,     -1, -1),
    MOUSEBIND_ENTRY(1107, &ID_Def_MouseBinds2, 7,  mouseb_arti_invulnerability, mouseb_arti_invulnerability2, -1, -1),
    MOUSEBIND_ENTRY(1108, &ID_Def_MouseBinds2, 8,  mouseb_arti_egg,             mouseb_arti_egg2,             -1, -1),
    MOUSEBIND_ENTRY(1109, &ID_Def_MouseBinds2, 9,  mouseb_arti_teleport,        mouseb_arti_teleport2,        -1, -1),
    MOUSEBIND_ENTRY(1110, &ID_Def_MouseBinds2, 10, mouseb_arti_teleportother,   mouseb_arti_teleportother2,   -1, -1),
    MOUSEBIND_ENTRY(1111, &ID_Def_MouseBinds2, 11, mouseb_arti_wings,           mouseb_arti_wings2,           -1, -1),
    MOUSEBIND_ENTRY(1112, &ID_Def_MouseBinds2, 12, mouseb_arti_servant,         mouseb_arti_servant2,         -1, -1),
    MOUSEBIND_ENTRY(1113, &ID_Def_MouseBinds2, 13, mouseb_arti_bracers,         mouseb_arti_bracers2,         -1, -1),

    MOUSEBIND_ENTRY(1200, &ID_Def_MouseBinds3, 0, mouseb_arti_boots,  mouseb_arti_boots2,  -1, -1),
    MOUSEBIND_ENTRY(1201, &ID_Def_MouseBinds3, 1, mouseb_arti_torch,  mouseb_arti_torch2,  -1, -1),
    MOUSEBIND_ENTRY(1202, &ID_Def_MouseBinds3, 2, mouseb_arti_krater, mouseb_arti_krater2, -1, -1),
    MOUSEBIND_ENTRY(1203, &ID_Def_MouseBinds3, 3, mouseb_arti_incant, mouseb_arti_incant2, -1, -1),
    MOUSEBIND_ENTRY(1204, &ID_Def_MouseBinds3, 4, mouseb_arti_all,    mouseb_arti_all2,    -1, -1),
};
...
static void M_DrawBindButton (int itemNum, int yPos, int btn1, int btn2)
{
    ...
    MN_DrTextAGlow(text, M_ItemRightAlign(text), yPos,
        CurrentItPos == itemNum && MouseIsBinding ? cr[CR_YELLOW] : (empty ? cr[CR_RED] : cr[CR_GREEN_HX]),
        CurrentItPos == itemNum && MouseIsBinding ? cr[CR_YELLOW_BRIGHT] : (empty ? cr[CR_RED_BRIGHT] : cr[CR_GREEN_HX_BRIGHT]),
        LINE_ALPHA(itemNum));
}
...
#define GAMEPADBIND_ENTRY(bindnum, menu_ptr, item_idx, slot_ref, def) \
    { bindnum, menu_ptr, item_idx, &(slot_ref), def }

static const GamepadBindEntry_t gamepadbinds[] =
{
    GAMEPADBIND_ENTRY(2000, &ID_Def_GamepadBinds,   2, joybfire,        0),
    GAMEPADBIND_ENTRY(2001, &ID_Def_GamepadBinds,   3, joybstrafe,      1),
    GAMEPADBIND_ENTRY(2002, &ID_Def_GamepadBinds,   4, joybuse,         2),
    GAMEPADBIND_ENTRY(2003, &ID_Def_GamepadBinds,   5, joybspeed,      -1),
    GAMEPADBIND_ENTRY(2004, &ID_Def_GamepadBinds,   6, joybjump,        8),
    GAMEPADBIND_ENTRY(2005, &ID_Def_GamepadBinds,   7, joybstrafeleft, -1),
    GAMEPADBIND_ENTRY(2006, &ID_Def_GamepadBinds,   8, joybstraferight,-1),
    GAMEPADBIND_ENTRY(2007, &ID_Def_GamepadBinds,   9, joybprevweapon,  9),
    GAMEPADBIND_ENTRY(2008, &ID_Def_GamepadBinds,  10, joybnextweapon, 10),
    GAMEPADBIND_ENTRY(2009, &ID_Def_GamepadBinds,  11, joybmenu,        6),
    GAMEPADBIND_ENTRY(2010, &ID_Def_GamepadBinds,  12, joybautomap,     3),
    GAMEPADBIND_ENTRY(2011, &ID_Def_GamepadBinds_2, 0, joybinvleft,    -1),
    GAMEPADBIND_ENTRY(2012, &ID_Def_GamepadBinds_2, 1, joybinvright,   -1),
    GAMEPADBIND_ENTRY(2013, &ID_Def_GamepadBinds_2, 2, joybuseartifact,-1),
    GAMEPADBIND_ENTRY(2014, &ID_Def_GamepadBinds_2, 3, joybflyup,      -1),
    GAMEPADBIND_ENTRY(2015, &ID_Def_GamepadBinds_2, 4, joybflydown,    -1),
    GAMEPADBIND_ENTRY(2016, &ID_Def_GamepadBinds_2, 5, joybflycenter,  -1),
};
...
// -----------------------------------------------------------------------------
// M_ClearGamepadBind
//  Clear gamepad bind on the line where cursor is placed (CurrentItPos).
// -----------------------------------------------------------------------------

static void M_ClearGamepadBind (int Current_ItPos)
{
    if (gamepad_enable && I_HasController())
    {
        for (size_t i = 0; i < sizeof(gamepadbinds) / sizeof(gamepadbinds[0]); i++)
        {
            if (gamepadbinds[i].menu == CurrentMenu
            && gamepadbinds[i].item == Current_ItPos)
            {
                *gamepadbinds[i].slot = -1;
                return;
            }
...
// -----------------------------------------------------------------------------
// M_DrawBindGamepad
//  Do gamepad button bind drawing.
// -----------------------------------------------------------------------------
...
static void M_DrawBindGamepad (int itemNum, int yPos, int btn)
{
    ...
    MN_DrTextAGlow(text, M_ItemRightAlign(text), yPos,
        CurrentItPos == itemNum && GamepadIsBinding ? cr[CR_YELLOW] : (empty ? cr[CR_RED] : cr[CR_GREEN_HX]),
        CurrentItPos == itemNum && GamepadIsBinding ? cr[CR_YELLOW_BRIGHT] : (empty ? cr[CR_RED_BRIGHT] : cr[CR_GREEN_HX_BRIGHT]),
        LINE_ALPHA(itemNum));
}
