...
// -----------------------------------------------------------------------------
// CL_MissionMatches
//  [PN] Matches mission token for Heretic parsing context.
//       Supported tokens: HERETIC, HTIC (or wildcard "*").
// -----------------------------------------------------------------------------
...
static boolean CL_MissionMatches(const char *token)
{
    ...
    if ((!strncasecmp(token, "HERETIC", 7) && token[7] == '\0')
