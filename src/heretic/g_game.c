...
//
    if (joyxmove < 0 || joyxmove > 0
        || gamekeydown[key_right] || gamekeydown[key_left] || gamekeydown[key_right2] || gamekeydown[key_left2])
        turnheld += ticdup;
    else
        turnheld = 0;
    ...
    if (gamekeydown[key_lookdown] || gamekeydown[key_lookdown2] || gamekeydown[key_lookup] || gamekeydown[key_lookup2])
    {
        lookheld += ticdup;
    }
    else
...
//
    ...
    if (gamekeydown[key_use] || gamekeydown[key_use2] || joybuttons[joybuse] || mouse_buttons(mousebuse, mousebuse2))
    {
        cmd->buttons |= BT_USE;
...
//
    ...
    if (widget_time)
    {
        const int time = (deathmatch && TimerGame ? TimerGame : leveltime);
