...
static void M_Bind_M_Bomb (int choice);
static void M_Bind_M_Tome (int choice);
static void M_Bind_M_Ring (int choice);
static void M_Bind_M_Chaosdevice (int choice);
static void M_Bind_M_Shadowsphere (int choice);
static void M_Bind_M_Wings (int choice);
static void M_Bind_M_Torch (int choice);
static void M_Bind_M_Morph (int choice);
...
static void M_ScrollPages (boolean direction)
{
    ...
    // Mouse bindings
    else if (CurrentMenu == &ID_Def_MouseBinds1) nextMenu = MENU_ID_MOUSEBINDS2;
    else if (CurrentMenu == &ID_Def_MouseBinds2) nextMenu = MENU_ID_MOUSEBINDS1;
...
static void M_Draw_ID_Video_2 (void)
{
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 60,
                        ...
                        post_homshimmer ? cr[CR_GREEN] : cr[CR_DARKRED],
                            V_PProc_EffectsActive() ? cr[CR_RED_BRIGHT] :
                            post_homshimmer ? cr[CR_GREEN_BRIGHT] : cr[CR_RED_BRIGHT],
                                LINE_ALPHA(4));
...
static void M_Bind_Weapon5 (int choice)
{
    M_StartBind(409);  // key_weapon5
}

static void M_Bind_Weapon6 (int choice)
{
    M_StartBind(410);  // key_weapon6
}

static void M_Bind_Weapon7 (int choice)
{
    M_StartBind(411);  // key_weapon7
}

static void M_Bind_PrevWeapon (int choice)
{
    M_StartBind(412);  // key_prevweapon
}

static void M_Bind_NextWeapon (int choice)
{
    M_StartBind(413);  // key_nextweapon
}
...
static void M_Draw_ID_Keybinds_8 (void)
{
    ...
    M_DrawBindKey(8, 100, key_multi_msgplayer[0], key_multi_msgplayer2[0]);
    M_DrawBindKey(9, 110, key_multi_msgplayer[1], key_multi_msgplayer2[1]);
    M_DrawBindKey(10, 120, key_multi_msgplayer[2], key_multi_msgplayer2[2]);
    M_DrawBindKey(11, 130, key_multi_msgplayer[3], key_multi_msgplayer2[3]);
...
static void M_Bind_ToPlayer1 (int choice)
{
    M_StartBind(807);  // key_multi_msgplayer[0]
}

static void M_Bind_ToPlayer2 (int choice)
{
    M_StartBind(808);  // key_multi_msgplayer[1]
}

static void M_Bind_ToPlayer3 (int choice)
{
    M_StartBind(809);  // key_multi_msgplayer[2]
}

static void M_Bind_ToPlayer4 (int choice)
{
    M_StartBind(810);  // key_multi_msgplayer[3]
}
...
static void M_Draw_ID_MouseBinds1 (void)
{
    ...
    M_DrawBindFooter("PAGE 1/2", true);
}
...
static void M_Bind_M_SpeedOn (int choice)        { M_StartMouseBind(1004); } // mousebspeed
static void M_Bind_M_StrafeOn (int choice)       { M_StartMouseBind(1005); } // mousebstrafe
static void M_Bind_M_StrafeLeft (int choice)     { M_StartMouseBind(1006); } // mousebstrafeleft
static void M_Bind_M_StrafeRight (int choice)    { M_StartMouseBind(1007); } // mousebstraferight
static void M_Bind_M_PrevWeapon (int choice)     { M_StartMouseBind(1008); } // mousebprevweapon
static void M_Bind_M_NextWeapon (int choice)     { M_StartMouseBind(1009); } // mousebnextweapon
static void M_Bind_M_InventoryLeft (int choice)  { M_StartMouseBind(1010); } // mousebinvleft
static void M_Bind_M_InventoryRight (int choice) { M_StartMouseBind(1011); } // mousebinvright
static void M_Bind_M_UseArtifact (int choice)    { M_StartMouseBind(1012); } // mousebuseartifact
...
static void M_DrawControllerName (void)
{
    ...
    MN_DrTextACentered("DETECTED CONTROLLER:", 145, cr[CR_YELLOW]);
    MN_DrTextACentered(controller_name, 155, controller_color);
}
...
static void M_Draw_ID_GamepadBinds (void)
{
    ...
    M_DrawBindGamepad(6, 80,  joybstrafeleft);
    M_DrawBindGamepad(7, 90,  joybstraferight);
    M_DrawBindGamepad(8, 100, joybprevweapon);
    M_DrawBindGamepad(9, 110, joybnextweapon);
    M_DrawBindGamepad(10, 120, joybmenu);
    M_DrawBindGamepad(11, 130, joybautomap);
...
static void M_Bind_G_StrafeLeft (int choice)   { M_StartGamepadBind(2004); }
static void M_Bind_G_StrafeRight (int choice)  { M_StartGamepadBind(2005); }
static void M_Bind_G_PrevWeapon (int choice)   { M_StartGamepadBind(2006); }
static void M_Bind_G_NextWeapon (int choice)   { M_StartGamepadBind(2007); }
static void M_Bind_G_Menu (int choice)         { M_StartGamepadBind(2008); }
static void M_Bind_G_Automap (int choice)      { M_StartGamepadBind(2009); }
...
static void M_Bind_G_InventoryLeft (int choice)  { M_StartGamepadBind(2010); }
static void M_Bind_G_InventoryRight (int choice) { M_StartGamepadBind(2011); }
static void M_Bind_G_UseArtifact (int choice)    { M_StartGamepadBind(2012); }
static void M_Bind_G_FlyUp (int choice)          { M_StartGamepadBind(2013); }
static void M_Bind_G_FlyDown (int choice)        { M_StartGamepadBind(2014); }
static void M_Bind_G_FlyCenter (int choice)      { M_StartGamepadBind(2015); }

static void M_Bind_G_Reset (int choice)
{
    MenuActive = false;
    askforquit = true;
    typeofask = 10;      // [PN] gamepad binds reset
}
...
static void M_Draw_ID_GamepadSettings_1 (void)
{
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 30, cr[CR_GREEN], cr[CR_GREEN_BRIGHT], LINE_ALPHA(1));

    M_snprintf(str, sizeof(str), "%d", joystick_move_sensitivity);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 40, cr[CR_GREEN], cr[CR_GREEN_BRIGHT], LINE_ALPHA(2));

    M_snprintf(str, sizeof(str), "%d", joystick_look_sensitivity);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 50, cr[CR_GREEN], cr[CR_GREEN_BRIGHT], LINE_ALPHA(3));
...
static void M_Draw_ID_GamepadSettings_2 (void)
{
    ...
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 60, cr[CR_GREEN], cr[CR_GREEN_BRIGHT], LINE_ALPHA(4));
    M_snprintf(str, sizeof(str), "%d", joystick_y_dead_zone);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 70, cr[CR_GREEN], cr[CR_GREEN_BRIGHT], LINE_ALPHA(5));
    M_snprintf(str, sizeof(str), "%d", joystick_strafe_dead_zone);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 80, cr[CR_GREEN], cr[CR_GREEN_BRIGHT], LINE_ALPHA(6));
    M_snprintf(str, sizeof(str), "%d", joystick_look_dead_zone);
    MN_DrTextAGlow(str, M_ItemRightAlign(str), 90, cr[CR_GREEN], cr[CR_GREEN_BRIGHT], LINE_ALPHA(7));
...
static void M_Draw_ID_Automap (void)
{
    ...
    MN_DrTextACentered("MINIMAP", 120, cr[CR_YELLOW]);
...
static const char *const QuitEndMsg[] = {
    ...
    "",                                            // [PN] typeofask 9 (savegame WAD mismatch), full text in drawer below
    "RESET GAMEPAD BINDINGS TO DEFAULT VALUES?",   // [PN] typeofask 10 (reset gamepad binds)
};
...
void MN_Drawer(void)
{
    ...
    if (MenuActive == false)
    {
        ...
        if (askforquit)
        {
            ...
            if (typeofask == 6 || typeofask == 7 || typeofask == 10)
            {
                M_FillBackground();
                      ...
                      const char *mapname)
{
    ...
    M_StringCopy(force_load_required_wad,
                 required_wad != NULL ? required_wad : "UNKNOWN",
                 sizeof(force_load_required_wad));
    M_StringCopy(force_load_mapname,
                 mapname != NULL ? mapname : "",
                 sizeof(force_load_mapname));
...
static boolean MN_ID_TypeOfAsk (void)
{
    switch (typeofask)
    {
        ...
        case 10: // [PN] Reset gamepad binds.
            M_ResetGamepadBinds();
            if (!netgame && !demoplayback)
            {
                paused = true;
        ...
        case 9: // [PN] Savegame WAD mismatch.
            if (force_load_available)
            {
                if (!netgame && !demoplayback)
                {
...
#undef JOY_BUTTON_MAPPED
    ...
    if (askforquit)
    {
        if (typeofask == 9 && !force_load_available)
        {
            if (event->type == ev_keydown
        ...
        || (key == key_menu_forward
         && (typeofask == 1 || typeofask == 2 || typeofask == 9))
        ...
        || (event->type == ev_mouse && event->data1 & 2))  // [JN] Cancel by right mouse button.
        {
            ...
            if (typeofask == 6 || typeofask == 7 || typeofask == 8 || typeofask == 9 || typeofask == 10)
            {
                if (!netgame && !demoplayback)
                {
                    paused = true;
    ...
    if (!FileMenuKeySteal)
    {
        ...
        else if (key == key_menu_del)
        {
            ...
            else if (CurrentMenu == &ID_Def_MouseBinds1 || CurrentMenu == &ID_Def_MouseBinds2)
            {
...
static const KeyBindEntry_t keybinds[] =
{
    ...
    // Keep legacy bindnum mapping from original switch (106=speed, 107=strafe).
    ...
    KEYBIND_ENTRY(409, &ID_Def_Keybinds_4, 10, key_weapon5,     key_weapon5_2,    '5', 0, KBS_GLOBAL),
    KEYBIND_ENTRY(410, &ID_Def_Keybinds_4, 11, key_weapon6,     key_weapon6_2,    '6', 0, KBS_GLOBAL),
    KEYBIND_ENTRY(411, &ID_Def_Keybinds_4, 12, key_weapon7,     key_weapon7_2,    '7', 0, KBS_GLOBAL),
    KEYBIND_ENTRY(412, &ID_Def_Keybinds_4, 13, key_prevweapon,  key_prevweapon2,  0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(413, &ID_Def_Keybinds_4, 14, key_nextweapon,  key_nextweapon2,  0,   0, KBS_GLOBAL),

    // Page 5
    KEYBIND_ENTRY(500, &ID_Def_Keybinds_5, 0, key_arti_quartz,       key_arti_quartz2,       0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(501, &ID_Def_Keybinds_5, 1, key_arti_urn,          key_arti_urn2,          0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(502, &ID_Def_Keybinds_5, 2, key_arti_bomb,         key_arti_bomb2,         0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(503, &ID_Def_Keybinds_5, 3, key_arti_tome,         key_arti_tome2,         127, 0, KBS_GLOBAL),
    KEYBIND_ENTRY(504, &ID_Def_Keybinds_5, 4, key_arti_ring,         key_arti_ring2,         0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(505, &ID_Def_Keybinds_5, 5, key_arti_chaosdevice,  key_arti_chaosdevice2,  0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(506, &ID_Def_Keybinds_5, 6, key_arti_shadowsphere, key_arti_shadowsphere2, 0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(507, &ID_Def_Keybinds_5, 7, key_arti_wings,        key_arti_wings2,        0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(508, &ID_Def_Keybinds_5, 8, key_arti_torch,        key_arti_torch2,        0,   0, KBS_GLOBAL),
    KEYBIND_ENTRY(509, &ID_Def_Keybinds_5, 9, key_arti_morph,        key_arti_morph2,        0,   0, KBS_GLOBAL),
    ...
    KEYBIND_ENTRY(807, &ID_Def_Keybinds_8, 8,  key_multi_msgplayer[0], key_multi_msgplayer2[0], 'g',        0, KBS_SENDTO_ONLY),
    KEYBIND_ENTRY(808, &ID_Def_Keybinds_8, 9,  key_multi_msgplayer[1], key_multi_msgplayer2[1], 'i',        0, KBS_SENDTO_ONLY),
    KEYBIND_ENTRY(809, &ID_Def_Keybinds_8, 10, key_multi_msgplayer[2], key_multi_msgplayer2[2], 'b',        0, KBS_SENDTO_ONLY),
    KEYBIND_ENTRY(810, &ID_Def_Keybinds_8, 11, key_multi_msgplayer[3], key_multi_msgplayer2[3], 'r',        0, KBS_SENDTO_ONLY),
};
...
static const MouseBindEntry_t mousebinds[] =
{
    ...
    MOUSEBIND_ENTRY(1004, &ID_Def_MouseBinds1, 4,  mousebspeed,       mousebspeed2,       -1, -1),
    MOUSEBIND_ENTRY(1005, &ID_Def_MouseBinds1, 5,  mousebstrafe,      mousebstrafe2,       1, -1),
    MOUSEBIND_ENTRY(1006, &ID_Def_MouseBinds1, 6,  mousebstrafeleft,  mousebstrafeleft2,  -1, -1),
    MOUSEBIND_ENTRY(1007, &ID_Def_MouseBinds1, 7,  mousebstraferight, mousebstraferight2, -1, -1),
    MOUSEBIND_ENTRY(1008, &ID_Def_MouseBinds1, 8,  mousebprevweapon,  mousebprevweapon2,   4, -1),
    MOUSEBIND_ENTRY(1009, &ID_Def_MouseBinds1, 9,  mousebnextweapon,  mousebnextweapon2,   3, -1),
    MOUSEBIND_ENTRY(1010, &ID_Def_MouseBinds1, 10, mousebinvleft,     mousebinvleft2,     -1, -1),
...
static const GamepadBindEntry_t gamepadbinds[] =
{
    ...
    GAMEPADBIND_ENTRY(2004, &ID_Def_GamepadBinds,   6, joybstrafeleft, -1),
    GAMEPADBIND_ENTRY(2005, &ID_Def_GamepadBinds,   7, joybstraferight,-1),
    GAMEPADBIND_ENTRY(2006, &ID_Def_GamepadBinds,   8, joybprevweapon,  9),
    GAMEPADBIND_ENTRY(2007, &ID_Def_GamepadBinds,   9, joybnextweapon, 10),
    GAMEPADBIND_ENTRY(2008, &ID_Def_GamepadBinds,  10, joybmenu,        6),
    GAMEPADBIND_ENTRY(2009, &ID_Def_GamepadBinds,  11, joybautomap,     3),
    GAMEPADBIND_ENTRY(2010, &ID_Def_GamepadBinds_2, 0, joybinvleft,    -1),
    GAMEPADBIND_ENTRY(2011, &ID_Def_GamepadBinds_2, 1, joybinvright,   -1),
    GAMEPADBIND_ENTRY(2012, &ID_Def_GamepadBinds_2, 2, joybuseartifact,-1),
    GAMEPADBIND_ENTRY(2013, &ID_Def_GamepadBinds_2, 3, joybflyup,      -1),
    GAMEPADBIND_ENTRY(2014, &ID_Def_GamepadBinds_2, 4, joybflydown,    -1),
    GAMEPADBIND_ENTRY(2015, &ID_Def_GamepadBinds_2, 5, joybflycenter,  -1),
};
...
static void M_DrawBindGamepad (int itemNum, int yPos, int btn)
{
    ...
    MN_DrTextAGlow(text, M_ItemRightAlign(text), yPos,
        CurrentItPos == itemNum && GamepadIsBinding ? cr[CR_YELLOW] : (empty ? cr[CR_RED] : cr[CR_GREEN]),
        CurrentItPos == itemNum && GamepadIsBinding ? cr[CR_YELLOW_BRIGHT] : (empty ? cr[CR_RED_BRIGHT] : cr[CR_GREEN_BRIGHT]),
        LINE_ALPHA(itemNum));
}
