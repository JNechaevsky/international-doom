...
void ID_RightWidgets (void)
{
    ...
    // [JN] Ammo widget. Don't draw outside of game level states.
    if (st_ammo_widget && gamestate == GS_LEVEL)
    {
        char str[8];
        player_t *player = &players[displayplayer];

        // Shift widgets down if no status bar background is used.
        int yy = dp_screen_size > 10 && (!automapactive || automap_overlay) ? 43 : 0;
        ...
        else
        {
            ...
            MN_DrTextA("W", 252 + WIDESCREENDELTA,  96 + yy, cr[CR_YELLOW]);
            MN_DrTextA("E", 252 + WIDESCREENDELTA, 106 + yy, cr[CR_GREEN]);
            MN_DrTextA("D", 252 + WIDESCREENDELTA, 116 + yy, cr[CR_BLUE2]);
            MN_DrTextA("H", 252 + WIDESCREENDELTA, 126 + yy, cr[CR_RED]);
            MN_DrTextA("P", 252 + WIDESCREENDELTA, 136 + yy, cr[CR_ORANGE]);
            MN_DrTextA("M", 252 + WIDESCREENDELTA, 146 + yy, cr[CR_LIGHTGRAY]);

            // Elven Wand
            sprintf(str, "%d/",  player->ammo[am_goldwand]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA - MN_TextAWidth(str), 96 + yy, cr[CR_GRAY]);
            sprintf(str, "%d",  player->maxammo[am_goldwand]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA, 96 + yy, cr[CR_GRAY]);

            // Ethereal Crossbow
            sprintf(str, "%d/",  player->ammo[am_crossbow]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA - MN_TextAWidth(str), 106 + yy, cr[CR_GRAY]);
            sprintf(str, "%d",  player->maxammo[am_crossbow]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA, 106 + yy, cr[CR_GRAY]);

            // Dragon Claw
            sprintf(str, "%d/",  player->ammo[am_blaster]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA - MN_TextAWidth(str), 116 + yy, cr[CR_GRAY]);
            sprintf(str, "%d",  player->maxammo[am_blaster]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA, 116 + yy, cr[CR_GRAY]);

            // Hellstaff
            sprintf(str, "%d/",  player->ammo[am_skullrod]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA - MN_TextAWidth(str), 126 + yy, cr[CR_GRAY]);
            sprintf(str, "%d",  player->maxammo[am_skullrod]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA, 126 + yy, cr[CR_GRAY]);

            // Phoenix Rod
            sprintf(str, "%d/",  player->ammo[am_phoenixrod]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA - MN_TextAWidth(str), 136 + yy, cr[CR_GRAY]);
            sprintf(str, "%d",  player->maxammo[am_phoenixrod]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA, 136 + yy, cr[CR_GRAY]);

            // Firemace
            sprintf(str, "%d/",  player->ammo[am_mace]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA - MN_TextAWidth(str), 146 + yy, cr[CR_GRAY]);
            sprintf(str, "%d",  player->maxammo[am_mace]);
            MN_DrTextA(str, 294 + WIDESCREENDELTA, 146 + yy, cr[CR_GRAY]);
