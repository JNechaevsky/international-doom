...
void ST_drawLine(int x, int y, int len, int color)
{
    ...
    pixel_t *drawpos = I_VideoBuffer + (y * vid_resolution) * SCREENWIDTH + ((x + WIDESCREENDELTA) * vid_resolution);
    int i = 0;

    while(i < (len * vid_resolution))
    {
        if (vid_resolution > 1)
#ifndef CRISPY_TRUECOLOR
...
boolean ST_DrawExternal(void)
{
    ...
    else
    {
        ...
        ST_drawNumFontY2(261, 132, keys);
         ...
         {
             V_DrawPatch(38, 86, W_CacheLumpName(DEH_String("CBOWA0"), PU_CACHE));
         }
         if(plyr->weaponowned[wp_rifle])
         {
             V_DrawPatch(40, 107, W_CacheLumpName(DEH_String("RIFLA0"), PU_CACHE));
         }
         if(plyr->weaponowned[wp_missile])
         {
             V_DrawPatch(39, 131, W_CacheLumpName(DEH_String("MMSLA0"), PU_CACHE));
         }
         if(plyr->weaponowned[wp_hegrenade])
         {
             V_DrawPatch(78, 87, W_CacheLumpName(DEH_String("GRNDA0"), PU_CACHE));
         }
         if(plyr->weaponowned[wp_flame])
         {
             V_DrawPatch(80, 117, W_CacheLumpName(DEH_String("FLAMA0"), PU_CACHE));
         }
         if(plyr->weaponowned[wp_mauler])
         {
             V_DrawPatch(75, 142, W_CacheLumpName(DEH_String("TRPDA0"), PU_CACHE));
         }
         ...
         {
             V_DrawPatch(280, 130, W_CacheLumpName(DEH_String("I_COMM"), PU_CACHE));
         }
    }
